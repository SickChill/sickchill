...
export default () => {
    ...
    const ifExists = (loopThroughArray, searchFor) => {
        ...
        loopThroughArray.forEach(rootObject => {
            ...
            log.info(`${rootObject.name} while searching for: ${searchFor}`);
        });
        return found;
    };
    ...
    $.fn.getCategories = function(isDefault, selectedProvider) {
        ...
        const params = {url, name, key};

        $('.updating_categories').wrapInner(`<span><img src="${srRoot}/images/loading16${config.themeSpinner}.gif"> Updating Categories ...</span>`);
        ...
        if ($('#provider_order_list > #' + id).length === 0 && showProvider !== false) {
            const toAdd = `
                <li class="ui-state-default" id="${id}">
                    ...
                    <a href="${config.anonUrl + url}" class="imgLink" target="_new">
                        <img src="${srRoot}/images/providers/newznab.png" alt="${name}" width="16" height="16">
                    </a> ${name}
                </li>
            `;
    ...
    $.fn.addTorrentRssProvider = function(id, name, url, cookies, titleTAG) {
        ...
        if ($('#provider_order_list > #' + id).length === 0) {
            $('#provider_order_list').append('<li class="ui-state-default" id="' + id + '"> <input type="checkbox" id="enable_' + id + '" class="provider_enabler" CHECKED> <a href="' + config.anonURL + url + '" class="imgLink" target="_new"><img src="' + srRoot + '/images/providers/torrentrss.png" alt="' + name + '" width="16" height="16"></a> ' + name + '</li>');
            $('#provider_order_list').sortable('refresh');
        }
    ...
    $.fn.populateNewznabSection = function() {
        ...
        if (selectedProvider === 'addNewznab') {
            ...
            $('#newznab_cat option').each((index, element) => $(element).remove());
            $('#newznab_cap option').each((index, element) => $(element).remove());
        } else {
    ...
    $.fn.makeTorrentRssProviderString = () => {
        const provStrings = [];
    ...
    $.fn.refreshProviderList = function() {
        ...
        const finalArr = [];
        $.each(idArr, (key, val) => {
            const checked = $('#enable_' + val).prop('checked') ? '1' : '0';
    ...
    $(document).on('change', '.newznab_key', (index, element) => {
        var providerId = $(element).attr('id');
        ...
        $(element).updateProvider(providerId, url, key, cat);
    });

    $(document).on('change', '#newznab_key,#newznab_url', (index, element) => {
        const selectedProvider = $('#editANewznabProvider :selected').val();
        ...
        $(element).updateProvider(selectedProvider, url, key, cat);
    });
    ...
    $('body').on('change', '#editAProvider', (index, element) => $(element).showHideProviders());
    $('body').on('change', '#editANewznabProvider', (index, element) => $(element).populateNewznabSection());
    $('body').on('change', '#editATorrentRssProvider', (index, element) => $(element).populateTorrentRssSection());
    $('body').on('change', '.provider_enabler', (index, element) => $(element).refreshProviderList());

    $('#newznab_cat_update').click(() => {
    ...
    $('#newznab_add').click(function() {
        ...
        $.getJSON(srRoot + '/config/providers/canAddNewznabProvider', {name}, function(data) {
    ...
    $('#torrentrss_add').click(function(){
        ...
        const cookies = $('#torrentrss_cookies').val();
        const titleTAG = $('#torrentrss_titleTAG').val();
        const params = {name, url, cookies, titleTAG};
    ...
    $.fn.makeTorrentOptionString = providerId => {
    ...
    $(this).on('change', '.seed_option', (index, element) => {
        const providerId = $(element).attr('id').split('_')[0];
        $(this).makeTorrentOptionString(providerId);
    });

    $.fn.replaceOptions = function(options) {
        const self = $(this).empty();

        options.forEach(option => {
            const optionElement = $('<option></option>').attr('value', option.value).text(option.text);
            self.append(optionElement);
        });
    };
