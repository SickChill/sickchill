...
function formtowizard(options) {
    this.setting = $.extend({persistsection: false, revealfx: ['slide', 500], oninit() {}, onpagechangestart() {}}, options);
    this.currentsection = -1;
    this.init(this.setting);
}

formtowizard.prototype = {
    createfieldsets($theform, arr) { // Reserved function for future version (dynamically wraps form elements with a fieldset element)
    ...
    loadsection(rawi, bypasshooks) {
      ...
      // Doload Boolean checks to see whether to load next section (true if bypasshooks param is true or onpagechangestart() event handler doesn't return false)
        let doload = bypasshooks || this.setting.onpagechangestart($, this.currentsection, this.sections.$sections.eq(this.currentsection));
        ...
        let i = (rawi === 'prev') ? this.currentsection - 1 : (rawi === 'next') ? this.currentsection + 1 : parseInt(rawi, 10); // Get index of next section to show
        ...
        if (i < this.sections.count && doload) { // If next section to show isn't the same as the current section shown
            ...
            if (this.setting.revealfx[0] === 'slide') {
                ...
                this.sections.$innerwrapper.stop().animate({left: -i * this.maxfieldsetwidth}, this.setting.revealfx[1], () => { // Slide next section into view
                    thiswizard.sections.$sections.each(thissec => {
                        if (thissec !== i) {
    ...
    addvalidatefields() {
        ...
        for (let i = 0; i < validatefields.length; i++) {
            const el = theform.elements[validatefields[i]]; // Reference form element
            if (el) { // If element is defined
                const $section = $(el).parents('fieldset.sectionwrap:eq(0)'); // Find fieldset.sectionwrap this form element belongs to
    ...
    validate(section) {
        const elements = this.sections.$sections.eq(section).data('elements'); // Reference elements within this section that should be validated
        const invalidtext = ['Please fill out the following fields:\n'];
        let validated = true;
    ...
    init(setting) {
        ...
        let $sectionswrapper_inner = null; // eslint-disable-line camelcase
        ...
        if (setting.revealfx[0] === 'slide') { // Create outer DIV that will house all the fieldset.sectionwrap elements
            ...
            $sectionswrapper_inner = $('<div style="position:absolute;left:0;top:0;"></div>'); // eslint-disable-line camelcase
        }
        ...
        if (setting.revealfx[0] === 'slide') {
            ...
            thiswizard.sections = {
                ...
                $innerwrapper: $sectionswrapper_inner, // eslint-disable-line camelcase
                $sections,
                count: $sections.length
            };
            thiswizard.sections.$sections.show();
        } else {
            thiswizard.sections = {$sections, count: $sections.length}; // Remember various parts of section container
        }
        ...
        thiswizard.paginatediv.$main.click(e => { // Assign behavior to pagination buttons
...
formtowizard.routines = {
    getCookie(Name) {
        const re = new RegExp(Name + '=[^;]+', 'i'); // Construct RE to search for target name/value pair
