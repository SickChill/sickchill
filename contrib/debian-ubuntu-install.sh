...
packages=$(dpkg -l unrar git-core openssl libssl-dev python3.8 2>1 | grep "no packages" | \
           awk '{ print $6 }' | tr '\n' ' ')
if [[ ${packages} ]]; then
...
if [[ $(dpkg -l unrar git-core openssl libssl-dev python3.8 2>&1 | grep "no packages" | \
        awk '{print $6 }') ]]; then
    whiptail --title "Package Installation Failed" --msgbox "               These Packages have failed:
               $(dpkg -l unrar git-core openssl libssl-dev python3.8 2>&1 | grep "no packages" | awk '{print $6 }')
