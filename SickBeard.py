...
# Do this before importing sickchill, to prevent locked files and incorrect import
...
signal.signal(signal.SIGINT, sickchill.sig_handler)
signal.signal(signal.SIGTERM, sickchill.sig_handler)
...
class SickChill(object):
    ...
    def __init__(self):
        ...
        sickchill.events = Events(self.shutdown)
        ...
        self.start_port = sickchill.WEB_PORT
        self.web_options = {}
    ...
    def clear_cache():
        ...
        """
        try:
            cache_folder = ek(os.path.join, sickchill.CACHE_DIR, 'mako')
    ...
    def help_message():
        ...
        """
        help_msg = __doc__
        help_msg = help_msg.replace('SickBeard.py', sickchill.MY_FULLNAME)
        help_msg = help_msg.replace('SickChill directory', sickchill.PROG_DIR)
    ...
    def start(self):  # pylint: disable=too-many-branches,too-many-statements
        ...
        if not sickchill.SYS_ENCODING or sickchill.SYS_ENCODING.lower() in ('ansi_x3.4-1968', 'us-ascii', 'ascii', 'charmap') or \
        ...
        try:
            ...
            sys.setdefaultencoding(sickchill.SYS_ENCODING)  # pylint: disable=no-member
        except (AttributeError, LookupError):
            sys.exit('Sorry, you MUST add the SickChill folder to the PYTHONPATH environment variable\n'
                     'or find another way to force Python to use {} for string encoding.'.format(sickchill.SYS_ENCODING))

        # Rename the main thread
        threading.currentThread().name = 'MAIN'

        args = SickChillArgumentParser(sickchill.PROG_DIR).parse_args()

        if args.force_update:
            result = self.force_update()
            sys.exit(int(not result))  # Ok -> 0 , Error -> 1

        # Need console logging for SickBeard.py and SickChill-console.exe
        sickchill.NO_RESIZE = args.noresize
        self.console_logging = (not hasattr(sys, 'frozen')) or (sickchill.MY_NAME.lower().find('-console') > 0) and not args.quiet
        ...
        sickchill.DATA_DIR = ek(os.path.abspath, args.datadir) if args.datadir else sickchill.DATA_DIR
        sickchill.CONFIG_FILE = ek(os.path.abspath, args.config) if args.config else ek(os.path.join, sickchill.DATA_DIR, 'config.ini')
        ...
        if not ek(os.access, sickchill.DATA_DIR, os.F_OK):
            try:
                ek(os.makedirs, sickchill.DATA_DIR, 0o744)
            except os.error:
                raise SystemExit('Unable to create data directory: {0}'.format(sickchill.DATA_DIR))

        # Make sure we can write to the data dir
        if not ek(os.access, sickchill.DATA_DIR, os.W_OK):
            raise SystemExit('Data directory must be writeable: {0}'.format(sickchill.DATA_DIR))

        # Make sure we can write to the config file
        if not ek(os.access, sickchill.CONFIG_FILE, os.W_OK):
            if ek(os.path.isfile, sickchill.CONFIG_FILE):
                raise SystemExit('Config file must be writeable: {0}'.format(sickchill.CONFIG_FILE))
            elif not ek(os.access, ek(os.path.dirname, sickchill.CONFIG_FILE), os.W_OK):
                raise SystemExit('Config file root dir must be writeable: {0}'.format(ek(os.path.dirname, sickchill.CONFIG_FILE)))

        ek(os.chdir, sickchill.DATA_DIR)

        # Check if we need to perform a restore first
        restore_dir = ek(os.path.join, sickchill.DATA_DIR, 'restore')
        if ek(os.path.exists, restore_dir):
            success = self.restore_db(restore_dir, sickchill.DATA_DIR)
            if self.console_logging:
                sys.stdout.write('Restore: restoring DB and config.ini {0}!\n'.format(('FAILED', 'SUCCESSFUL')[success]))

        # Load the config and publish it to the sickchill package
        if self.console_logging and not ek(os.path.isfile, sickchill.CONFIG_FILE):
            sys.stdout.write('Unable to find {0}, all settings will be default!\n'.format(sickchill.CONFIG_FILE))

        sickchill.CFG = ConfigObj(sickchill.CONFIG_FILE, encoding='UTF-8')

        # Initialize the config and our threads
        sickchill.initialize(consoleLogging=self.console_logging)

        if self.run_as_daemon:
            self.daemonize()

        # Get PID
        sickchill.PID = os.getpid()

        # Build from the DB to start with
        self.load_shows_from_db()

        logger.log('Starting SickChill [{branch}] using \'{config}\''.format
                   (branch=sickchill.BRANCH, config=sickchill.CONFIG_FILE))
        ...
        if sickchill.WEB_LOG:
            self.log_dir = sickchill.LOG_DIR
        else:
            self.log_dir = None

        # sickchill.WEB_HOST is available as a configuration value in various
        # places but is not configurable. It is supported here for historic reasons.
        if sickchill.WEB_HOST and sickchill.WEB_HOST != '0.0.0.0':
            self.web_host = sickchill.WEB_HOST
        else:
            self.web_host = '' if sickchill.WEB_IPV6 else '0.0.0.0'

        # web server options
        self.web_options = {
            'port': int(self.start_port),
            'host': self.web_host,
            'data_root': ek(os.path.join, sickchill.PROG_DIR, 'gui', sickchill.GUI_NAME),
            'web_root': sickchill.WEB_ROOT,
            'log_dir': self.log_dir,
            'username': sickchill.WEB_USERNAME,
            'password': sickchill.WEB_PASSWORD,
            'enable_https': sickchill.ENABLE_HTTPS,
            'handle_reverse_proxy': sickchill.HANDLE_REVERSE_PROXY,
            'https_cert': ek(os.path.join, sickchill.PROG_DIR, sickchill.HTTPS_CERT),
            'https_key': ek(os.path.join, sickchill.PROG_DIR, sickchill.HTTPS_KEY),
        }
        ...
        # sure, why not?
        if sickchill.USE_FAILED_DOWNLOADS:
            failed_history.trimHistory()

        # Check for metadata indexer updates for shows (sets the next aired ep!)
        # sickchill.showUpdateScheduler.forceRun()

        # Launch browser
        if sickchill.LAUNCH_BROWSER and not (self.no_launch or self.run_as_daemon):
            sickchill.launchBrowser('https' if sickchill.ENABLE_HTTPS else 'http', self.start_port, sickchill.WEB_ROOT)
    ...
    def load_shows_from_db():
        ...
        for sql_show in sql_results:
            try:
                ...
                sickchill.showList.append(cur_show)
    ...
    def restore_db(src_dir, dst_dir):
        ...
        """
        try:
            files_list = ['sickchill.db', 'config.ini', 'failed.db', 'cache.db']
    ...
    def shutdown(self, event):
        ...
        if sickchill.started:
            sickchill.halt()  # stop all tasks
            sickchill.saveAll()  # save all shows to DB
            ...
            if event == sickchill.event_queue.Events.SystemEvent.RESTART:
                install_type = sickchill.versionCheckScheduler.action.install_type

                popen_list = []

                if install_type in ('git', 'source'):
                    popen_list = [sys.executable, sickchill.MY_FULLNAME]
                elif install_type == 'win':
                    logger.log('You are using a binary Windows build of SickChill. '
                               'Please switch to using git.', logger.ERROR)

                if popen_list and not sickchill.NO_RESTART:
                    popen_list += sickchill.MY_ARGS
    ...
    def force_update():
        ...
        if ek(os.path.isdir, ek(os.path.join, sickchill.PROG_DIR, '.git')):  # update with git
