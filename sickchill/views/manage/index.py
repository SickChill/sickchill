...
from sickchill.sickbeard import db, logger, subtitles as subtitle_module, ui
...
class Manage(Home, WebRoot):
    ...
    def showEpisodeStatuses(indexer_id, whichStatus):
        ...
        cur_show_results = main_db_con.select(
            "SELECT season, episode, name FROM tv_episodes WHERE showid = ? AND season != 0 AND status IN ({0})".format(','.join(
    ...
    def episodeStatuses(self, whichStatus=None):
        ...
        status_results = main_db_con.select(
            ...
            "tv_episodes.showid = tv_shows.indexer_id ORDER BY show_name".format(','.join(['?'] * len(status_list))), status_list)
