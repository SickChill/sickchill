...
def get_scene_exceptions(indexer_id, season=-1):
    ...
    logger.database(f"get_scene_exceptions: {response}")
    logger.database(f"exceptions_cache: {exceptions_cache.get(indexer_id)}")
    return response
...
def get_all_scene_exceptions(indexer_id):
    ...
    logger.database(f"get_all_scene_exceptions: {all_exceptions_dict}")
    logger.database(f"exceptions_cache for {indexer_id}: {exceptions_cache.get(indexer_id)}")
