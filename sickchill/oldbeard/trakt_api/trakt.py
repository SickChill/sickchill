...
def refresh_trakt_pin_url() -> None:
    """Rebuild the legacy PIN authorize URL from the active Client ID.

    Called on startup and whenever Trakt Client ID/Secret change so the Get Trakt PIN
    button always opens an authorize URL that matches the configured application.
    """
    client_id = settings.TRAKT_API_KEY or ""
    settings.TRAKT_PIN_URL = f"https://trakt.tv/oauth/authorize?response_type=code&client_id={client_id}&redirect_uri={_OOB_REDIRECT_URI}"


def trakt_credentials_configured() -> bool:
    """Return True when the user has supplied a Trakt VIP API app Client ID and Secret."""
    return bool((settings.TRAKT_API_KEY or "").strip() and (settings.TRAKT_API_SECRET or "").strip())


def clear_revoked_trakt_defaults() -> bool:
    """Blank known-revoked bundled Client IDs still present in an old config.ini.

    Returns True if credentials were cleared.
    """
    key = (settings.TRAKT_API_KEY or "").strip()
    if key and key in getattr(settings, "TRAKT_REVOKED_CLIENT_IDS", ()):
    ...
    def traktRequest(
        ...
        if code == 401:
            ...
            if oauth_error in {"invalid_grant"}:
                ...
                raise traktTokenExpiredException(_("Trakt refresh token invalid - please reauthorize SickChill in the Trakt settings."))
