...
def delete_folder(folder: Path, check_empty=True):
    """
    ...
    if settings.TV_DOWNLOAD_DIR and folder.resolve() == Path(settings.TV_DOWNLOAD_DIR).resolve():
        return False

    # check if it's empty folder when wanted checked
    if check_empty:
        check_files = sorted(folder.iterdir())
        if check_files:
            logger.info(_(f"Not deleting folder {folder} found the following files: {check_files}"))
            return False

        try:
            logger.info(_(f"Deleting folder (if it's empty): {folder}"))
            folder.rmdir()
        except (OSError, IOError) as error:
            logger.warning(_(f"Warning: unable to delete folder: {folder}: {error}"))
            return False
    else:
        try:
...
def delete_files(process_path: Path, unwanted_files, result, force=False):
    """
    ...
    if not result.result and force:
        result.output += log_helper(_("Forcing deletion of files, even though last result was not success"), logger.DEBUG)
    elif not result.result:
        return

    # Delete all file not needed
    for cur_file in unwanted_files:
        cur_file_path = process_path / cur_file
        if not cur_file_path.is_file():
            continue  # Prevent error when a notwantedfiles is an associated files

        result.output += log_helper(_(f"Deleting file: {cur_file}"), logger.DEBUG)

        # check first the read-only attribute
        if not cur_file_path.stat().st_mode & stat.S_IWRITE:
            # File is read-only, so make it writeable
            result.output += log_helper(_(f"Changing ReadOnly Flag for file: {cur_file}"), logger.DEBUG)
            try:
                cur_file_path.chmod(stat.S_IWRITE)
            except OSError as error:
                result.output += log_helper(_(f"Cannot change permissions of {cur_file_path}: {error}"), logger.DEBUG)
        try:
            cur_file_path.unlink()
        except OSError as error:
            result.output += log_helper(_(f"Unable to delete file {cur_file}: {error}"), logger.DEBUG)


def process_dir(process_path: Path, release_name=None, process_method=None, force=False, is_priority=None, delete_on=False, failed=False, mode="auto"):
    """
    ...
    try:
        ...
        if process_path.is_dir():
            ...
            result.output += log_helper(_(f"Processing in folder {process_path}"), logger.DEBUG)

        # if the client and SickChill are not on the same machine translate the directory into a network directory
        elif all(
            [settings.TV_DOWNLOAD_DIR, settings.TV_DOWNLOAD_DIR.is_dir(), process_path.resolve() == settings.TV_DOWNLOAD_DIR.resolve()]
        ):
            process_path = settings.TV_DOWNLOAD_DIR / process_path.name
            result.output += log_helper(_(f"Trying to use folder: {process_path} "), logger.DEBUG)
        ...
        for current_directory, directory_names, filenames in generator_to_use:
            ...
            delete_folder(current_directory / "@eaDir", False)
...
def validate_dir(process_path, release_name, failed, result):
    ...
    if upper_name.startswith("_FAILED_") or upper_name.endswith("_FAILED_") or (os.sep + "_FAILED_") in upper_name or ("_FAILED_" + os.sep) in upper_name:
        ...
        result.missed_files.append(_(f"{process_path} : Being unpacked"))
        return False

    if failed:
        process_failed(process_path, release_name, result)
        result.missed_files.append(_(f"{process_path} : Failed download"))
        return False

    if settings.TV_DOWNLOAD_DIR and process_path.resolve() != settings.TV_DOWNLOAD_DIR.resolve() and helpers.is_hidden_folder(process_path):
        result.output += log_helper(_(f"Ignoring hidden folder: {process_path}"), logger.DEBUG)
        if not process_path.endswith("@eaDir"):
            result.missed_files.append(_(f"{process_path} : Hidden folder"))
        return False
    ...
    for current_directory, directory_names, filenames in os.walk(process_path, topdown=False, followlinks=settings.PROCESSOR_FOLLOW_SYMLINKS):
        ...
        if sync_files and settings.POSTPONE_IF_SYNC_FILES:
            result.output += log_helper(_(f"Found temporary sync files: {sync_files} in path: {process_path / sync_files[0]}"))
            result.output += log_helper(_(f"Skipping post processing for folder: {process_path}"))
            result.missed_files.append(_(f"{process_path / sync_files[0]} : Sync files found"))
            continue
        ...
        for found_file in found_files:
            try:
                ...
                logger.debug(_(f"Could not properly parse a show and episode from [{found_file}]: {error}"))
            else:
                return True

    result.output += log_helper(_(f"{process_path} : No processable items found in folder"), logger.DEBUG)
    return False
...
def unrar(path, rar_files, force, result):
    ...
    if settings.UNPACK == settings.UNPACK_PROCESS_CONTENTS and rar_files:
        result.output += log_helper(_(f"Packed Releases detected: {rar_files}"), logger.DEBUG)
        for archive in rar_files:
            ...
            try:
                ...
                if already_processed(path, archive, force, result):
                    result.output += log_helper(_(f"Archive file already post-processed, extraction skipped: {archive_path}"), logger.DEBUG)
                    continue

                if not helpers.is_rar_file(archive_path):
                    continue

                result.output += log_helper(_(f"Checking if archive is valid and contains a video: {archive_path}"), logger.DEBUG)
                ...
                if rar_handle.needs_password():
                    ...
                    result.output += log_helper(_(f"Archive needs a password, skipping: {archive_path}"))
                    continue
                ...
                if settings.UNPACK_DIR and os.path.isdir(settings.UNPACK_DIR):  # verify the unpack dir exists
                    ...
                    if settings.UNPACK_DIR:  # Let user know if we can't unpack there
                        result.output += log_helper(_(f"Unpack directory cannot be verified. Using {path}"), logger.DEBUG)

                # Fix up the list for checking if already processed
                rar_media_files = [(unpack_base_dir / rar_release_name / rar_media_file) for rar_media_file in rar_media_files]
                ...
                for rar_media_file in rar_media_files:
                    ...
                    if already_processed(check_path, check_file, force, result):
                        result.output += log_helper(_(f"Archive file already post-processed, extraction skipped: {rar_media_file}"), logger.DEBUG)
                        skip_rar = True
                        break
                ...
                result.output += log_helper(_(f"Unpacking archive: {archive}"), logger.DEBUG)
            ...
            if failure:
                result.output += log_helper(_(f"Failed to extract the archive {archive}: {failure[0]}"), logger.WARNING)
                result.missed_files.append(_(f"{archive} : Unpacking failed: {failure[1]}"))
                result.result = False
                continue
...
def already_processed(process_path, video_file, force, result):
    ...
    statuses_string = ",".join([str(x) for x in common.Quality.DOWNLOADED + common.Quality.ARCHIVED])
    search_sql += f" AND tv_episodes.status IN ({statuses_string})"
    search_sql += " AND history.resource LIKE ? LIMIT 1"
    sql_result = main_db_con.select(search_sql, [f"%{video_file}"])
    if sql_result:
...
def process_media(process_path, video_files, release_name, process_method, force, is_priority, result):
    ...
    for cur_video_file in video_files:
        cur_video_file_path = Path(process_path) / cur_video_file

        if already_processed(process_path, cur_video_file, force, result):
            result.output += log_helper(_(f"Skipping already processed file: {cur_video_file}"), logger.DEBUG)
            continue
        ...
        if result.result:
            result.output += log_helper(_(f"Processing succeeded for {cur_video_file_path}"))
        else:
            result.output += log_helper(_(f"Processing failed for {cur_video_file_path}: {process_fail_message}"), logger.WARNING)
...
def process_failed(process_path, release_name, result):
    ...
    if settings.DELETE_FAILED and result.result:
        if delete_folder(process_path, check_empty=False):
            result.output += log_helper(_(f"Deleted folder: {process_path}"), logger.DEBUG)

    if result.result:
        result.output += log_helper(_(f"Failed Download Processing succeeded: ({release_name}, {process_path})"))
    else:
        result.output += log_helper(
            _(f"Failed Download Processing failed: ({release_name}, {process_path}): {process_fail_message}"),
            logger.WARNING
        )
...
def subtitles_enabled(video):
    ...
    try:
        ...
        logger.warning(_(f"Not enough information to parse filename into a valid show. Consider add scene exceptions or improve naming for: {video}"))
        return False
    ...
    if parse_result.show.indexerid:
        ...
        logger.warning(_(f"Empty indexer ID for: {video}"))
        return False
