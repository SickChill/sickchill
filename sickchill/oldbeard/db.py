...
class DBConnection(object):
    ...
    def _error_log_helper(self, exception, severity, local_variables, attempts, called_method):
        if attempts in (0, self.MAX_ATTEMPTS):  # Only log the first try and the final failure
            log_level = ("Database", "Fatal")[severity == logger.ERROR]
            # noinspection PyUnresolvedReferences
            logger.log(
                severity,
                _(f"{log_level} error executing query with {called_method} in database {self.full_path}: {exception}")
            )
                ...
                try:
                    ...
                    for qu in query_list:
                        if len(qu) == 1:
                            # noinspection PyUnresolvedReferences
                            query = qu[0]
                            logger.log(log_level, _(f"{self.filename}: {query}"))
                            sql_results.append(self._execute(qu[0], fetchall=fetchall))
                        elif len(qu) > 1:
                            # noinspection PyUnresolvedReferences
                            query = qu[0]
                            args = qu[1]
                            logger.log(log_level, _(f"{self.filename}: {query} with args {args}"))
        ...
        with db_locks[self.filename]:
            ...
            for attempt in range(0, self.MAX_ATTEMPTS):
                try:
                    if settings.DBDEBUG:
                        if args is None:
                            logger.log(logger.DB, f"{self.filename}: {query}")
                        else:
    ...
    def mass_upsert(self, table, query_list, log_transaction=False):
        # type: (str, List[tuple, list], bool) -> None
        """
        Execute multiple queries

        :param table: name of table to upsert
        :param query_list: list of queries
        :param log_transaction: Boolean to wrap all in one transaction
        :return: None
        """
        ...
        for values, control in query_list:
            ...
            self.upsert(table, values, control)

    def upsert(self, table, value_dict, key_dict):
        """
        Update values, or if no updates done, insert values
        TODO: Make this return true/false on success/error

        :param table: table to update/insert
        ...
        if key_dict:
            ...
            # language=TEXT
            pairs = make_string(value_dict, ", ")
            control=make_string(key_dict, " AND ")
            query = f"UPDATE [{table}] SET {pairs} WHERE {control}"
    ...
    def table_info(self, table):
        ...
        """
        return {column["name"]: {"type": column["type"]} for column in self.select("PRAGMA table_info(`{0}`)".format(table))}
    ...
    def has_table(self, table):
        ...
        """
        return len(self.select("SELECT 1 FROM sqlite_master WHERE name = ?;", (table,))) > 0
    ...
    def has_column(self, table, column):
        ...
        """
        return column in self.table_info(table)
    ...
    def add_column(self, table, column, col_type="NUMERIC", default=0):
        ...
        self.action(f"UPDATE [{table}] SET {column} = ?", (default,))
...
def _process_upgrade(connection, upgrade_class):
    ...
    # logger.debug(f"Checking {pretty_name(upgrade_class.__name__)} database upgrade")
    if not instance.test():
        upgrade_name = pretty_name(upgrade_class.__name__)
        logger.debug(f"Database upgrade required: {upgrade_name}")
        try:
            instance.execute()
        except Exception as error:
...
class SchemaUpgrade(object):
    ...
    def has_table(self, table):
        return self.connection.has_table(table=table)

    def has_column(self, table, column):
        return self.connection.has_column(table=table, column=column)
    ...
    def increment_db_version(self):
        ...
        self.connection.action("UPDATE db_version SET db_version = ? WHERE 1", [new_version])
        return new_version
    ...
    def inc_major_version(self):
        ...
        major_version += 1
        minor_version = 0
        self.connection.action("UPDATE db_version SET db_version = ?, db_minor_version = ? WHERE 1", [major_version, minor_version])
