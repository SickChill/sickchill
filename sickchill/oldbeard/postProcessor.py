        ...
        if not priority_download:
            ...
            if int(ep_obj.season) > 0:
                ...
                if not isinstance(max_season[0][0], int) or max_season[0][0] < 0:
                    self._log(
                        f"File has season {ep_obj.season}, while the database does not have any known seasons yet. Try forcing a full update on the show and process this file again. The file may be incorrectly labeled or fake, aborting."
                    )
                    return False
                ...
                if int(ep_obj.season) > max_season[0][0]:
                    self._log(
                        _(
                            f"File has season {ep_obj.season}, while the indexer is on season {max_season[0][0]}. Try forcing a full update on the show and process this file again. The file may be incorrectly labeled or fake, aborting."
                        )
                    )
                    return False
