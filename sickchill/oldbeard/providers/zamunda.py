...
class Provider(TorrentProvider):
    ...
    def search(self, search_strings, age=0, ep_obj=None):
        ...
        for mode in search_strings:
            ...
            logger.debug(("Search Mode: {mode}").format(mode=mode))
            for search_string in {*search_strings[mode]}:
                
                search_url = self.urls["search"] % quote_plus(search_string) + self.categories
                logger.debug(("Search String: {search_string}").format(search_string=search_string))
                ...
                try:
                    ...
                    logger.debug("Could not find table of torrents zbtable")
                    continue
