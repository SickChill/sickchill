...
def download_subtitles(episode, force_lang=None):
    ...
    if not needs_subtitles(existing_subtitles, force_lang):
        logger.debug(
            f"Episode already has all needed subtitles, skipping {episode.show.name} {episode_num(episode.season, episode.episode) or episode_num(episode.season, episode.episode, numbering='absolute')}"
        )
    ...
    if not languages:
        logger.debug(
            f"No subtitles needed for {episode.show.name} {episode_num(episode.season, episode.episode) or episode_num(episode.season, episode.episode, numbering='absolute')}"
        )
        return existing_subtitles, None
    ...
    if not video:
        logger.debug(
            f"Exception caught in subliminal.scan_video for {episode.show.name} {episode_num(episode.season, episode.episode) or episode_num(episode.season, episode.episode, numbering='absolute')}"
        )
        return existing_subtitles, None
    ...
    try:
        ...
        if not subtitles_list:
            logger.debug(
                f"No subtitles found for {episode.show.name} {episode_num(episode.season, episode.episode) or episode_num(episode.season, episode.episode, numbering='absolute')}"
            )
            return existing_subtitles, None
...
def refresh_subtitles(episode):
    ...
    if episode.subtitles == current_subtitles:
        logger.debug(
            f"No changed subtitles for {episode.show.name} {episode_num(episode.season, episode.episode) or episode_num(episode.season, episode.episode, numbering='absolute')}"
        )
        return episode.subtitles, None
    else:
...
class SubtitlesFinder(object):
    ...
    def run(self, force=False):
        ...
        sql_results = database.select(
            ...
            + f"AND e.location != '' AND e.status IN ({','.join(['?'] * len(Quality.DOWNLOADED))}) ORDER BY age ASC",
        ...
        for ep_to_sub in sql_results:
            if not os.path.isfile(ep_to_sub["location"]):
                logger.debug(
                    f"Episode file does not exist, cannot download subtitles for {ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')}"
                )
                continue

            if not needs_subtitles(ep_to_sub["subtitles"]):
                logger.debug(
                    f"Episode already has all needed subtitles, skipping {ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')}"
                )
                continue
            ...
            try:
                if not force:
                    ...
                    if lastsearched + delay_time > now and int(ep_to_sub["searchcount"]) > 2 and days:
                        logger.debug(
                            f"Subtitle search for {ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')} delayed for {dhm(lastsearched + delay_time - now)}"
                        )
                        continue

                logger.info(
                    f"Searching for missing subtitles of {ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')}"
                )

                show_object = Show.find(settings.showList, int(ep_to_sub["showid"]))
                if not show_object:
                    logger.debug(f"Show with ID {ep_to_sub['showid']} not found in the database")
                    continue

                episode_object = show_object.getEpisode(ep_to_sub["season"], ep_to_sub["episode"])
                if isinstance(episode_object, str):
                    logger.debug(
                        f"{ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')} not found in the database"
                    )
                    continue
                ...
                try:
                    ...
                    logger.error(
                        f"Unable to find subtitles for {ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')}. Error: {str(error)}"
                    )
                    continue

                if new_subtitles:
                    logger.info(
                        f"Downloaded {', '.join(new_subtitles)} subtitles for {ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')}"
                    )

            except Exception as error:
                logger.error(
                    f"Error while searching subtitles for {ep_to_sub['show_name']} {episode_num(ep_to_sub['season'], ep_to_sub['episode']) or episode_num(ep_to_sub['season'], ep_to_sub['episode'], numbering='absolute')}. Error: {str(error)}"
                )
                continue
...
def run_subs_extra_scripts(episode, subtitle, video, single=False):
    for script_name in settings.SUBTITLES_EXTRA_SCRIPTS:
        ...
        try:
            ...
            logger.debug(f"Script result: {str(stdout or stderr).strip()}")

        except Exception as error:
            logger.info(f"Unable to run subs_extra_script: {str(error)}")
...
def refine_video(video, episode):
    ...
    for name in metadata_mapping:
        try:
            ...
            logger.debug(f"Unable to set {type(video)}.{name} from episode.{metadata_mapping[name]} attribute")
