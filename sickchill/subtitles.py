...
class SubtitleProviderPool(object):  # pylint: disable=too-few-public-methods
    ...
    def __init_instance():
        with SubtitleProviderPool._lock:
            ...
            provider_configs = {
                'addic7ed': {
                    'username': sickchill.ADDIC7ED_USER,
                    'password': sickchill.ADDIC7ED_PASS
                },
                'itasa': {
                    'username': sickchill.ITASA_USER,
                    'password': sickchill.ITASA_PASS
                },
                'legendastv': {
                    'username': sickchill.LEGENDASTV_USER,
                    'password': sickchill.LEGENDASTV_PASS
                },
                'opensubtitles': {
                    'username': sickchill.OPENSUBTITLES_USER,
                    'password': sickchill.OPENSUBTITLES_PASS
                },
                'subscenter': {
                    'username': sickchill.SUBSCENTER_USER,
                    'password': sickchill.SUBSCENTER_PASS
                }
            }
...
def sorted_service_list():
    ...
    current_index = 0
    for current_service in sickchill.SUBTITLES_SERVICES_LIST:
        if current_service in provider_manager.names():
            new_list.append({'name': current_service,
                             ...
                             'enabled': sickchill.SUBTITLES_SERVICES_ENABLED[current_index] == 1})
        current_index += 1
...
def wanted_languages(sql_like=None):
    wanted = frozenset(sickchill.SUBTITLES_LANGUAGES).intersection(subtitle_code_filter())
    return (wanted, '%' + ','.join(sorted(wanted)) + '%' if sickchill.SUBTITLES_MULTI else '%und%')[bool(sql_like)]


def get_needed_languages(subtitles):
    if not sickchill.SUBTITLES_MULTI:
...
def download_subtitles(episode, force_lang=None):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
    ...
    user_score = 213 if sickchill.SUBTITLES_PERFECT_MATCH else 198
    ...
    try:
        ...
        for subtitle in subtitles_list:
            score = subliminal.score.compute_score(subtitle, video,
                                                   hearing_impaired=sickchill.SUBTITLES_HEARING_IMPAIRED)
        ...
        found_subtitles = pool.download_best_subtitles(subtitles_list, video, languages=languages,
                                                       hearing_impaired=sickchill.SUBTITLES_HEARING_IMPAIRED,
                                                       min_score=user_score, only_one=not sickchill.SUBTITLES_MULTI)

        subliminal.save_subtitles(video, found_subtitles, directory=subtitles_path,
                                  single=not sickchill.SUBTITLES_MULTI)
    ...
    for subtitle in found_subtitles:
        subtitle_path = subliminal.subtitle.get_subtitle_path(video.name,
                                                              None if not sickchill.SUBTITLES_MULTI else
                                                              subtitle.language)
        if subtitles_path is not None:
            subtitle_path = os.path.join(subtitles_path, os.path.split(subtitle_path)[1])

        sickchill.helpers.chmodAsParent(subtitle_path)
        sickchill.helpers.fixSetGroupID(subtitle_path)

        if sickchill.SUBTITLES_HISTORY:
        ...
        if sickchill.SUBTITLES_EXTRA_SCRIPTS and is_media_file(video_path) and not sickchill.EMBEDDED_SUBTITLES_ALL:

            run_subs_extra_scripts(episode, subtitle, video, single=not sickchill.SUBTITLES_MULTI)
    ...
    if not sickchill.SUBTITLES_MULTI and len(found_subtitles) == 1:
...
def get_video(video_path, subtitles_path=None, subtitles=True, embedded_subtitles=None, episode=None):
    ...
    try:
        ...
        video_path = video_path.encode(sickchill.SYS_ENCODING)
        subtitles_path = subtitles_path.encode(sickchill.SYS_ENCODING)
    ...
    try:
        ...
        if embedded_subtitles is None:
            embedded_subtitles = bool(not sickchill.EMBEDDED_SUBTITLES_ALL and video_path.endswith('.mkv'))
...
def get_subtitles_path(video_path):
    if os.path.isabs(sickchill.SUBTITLES_DIR):
        new_subtitles_path = sickchill.SUBTITLES_DIR
    elif sickchill.SUBTITLES_DIR:
        new_subtitles_path = os.path.join(os.path.dirname(video_path), sickchill.SUBTITLES_DIR)
        dir_exists = sickchill.helpers.makeDir(new_subtitles_path)
        if not dir_exists:
            logger.log('Unable to create subtitles folder {0}'.format(new_subtitles_path), logger.ERROR)
        else:
            sickchill.helpers.chmodAsParent(new_subtitles_path)
    else:
    ...
    try:
        ...
        # Fallback to system encoding. This should never happen.
        new_subtitles_path = new_subtitles_path.encode(sickchill.SYS_ENCODING)
...
class SubtitlesFinder(object):  # pylint: disable=too-few-public-methods
    ...
    def run(self, force=False):  # pylint: disable=too-many-branches, too-many-statements, too-many-locals
        if not sickchill.USE_SUBTITLES:
            return

        if not sickchill.subtitles.enabled_service_list():
        ...
        sql_results = database.select(
            ...
            + ("AND e.season != 0 ", "")[sickchill.SUBTITLES_INCLUDE_SPECIALS]
        ...
        for ep_to_sub in sql_results:
            try:
                ...
                subtitle_path = ep_to_sub[b'location'].encode(sickchill.SYS_ENCODING)
            except UnicodeEncodeError:
                # Fallback to UTF-8.
            ...
            try:
                ...
                show_object = Show.find(sickchill.showList, int(ep_to_sub[b'showid']))
                if not show_object:
...
def run_subs_extra_scripts(episode, subtitle, video, single=False):
    for script_name in sickchill.SUBTITLES_EXTRA_SCRIPTS:
        ...
        try:
            process = subprocess.Popen(inner_cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                                       stderr=subprocess.STDOUT, cwd=sickchill.PROG_DIR)
