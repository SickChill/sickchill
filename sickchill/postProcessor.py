...
class PostProcessor(object):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, file_path, nzb_name=None, process_method=None, is_priority=None):
        ...
        self.process_method = process_method if process_method else sickchill.PROCESS_METHOD
    ...
    def list_associated_files(  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        def recursive_glob(treeroot, pattern):
            results = []
            for base, dirnames_, files in ek(os.walk, treeroot.encode(sickchill.SYS_ENCODING), followlinks=sickchill.PROCESSOR_FOLLOW_SYMLINKS):
        ...
        for associated_file_path in filelist:
            ...
            if ek(os.path.isfile, associated_file_path):
                ...
                if sickchill.MOVE_ASSOCIATED_FILES and associated_file_path.endswith(tuple(sickchill.ALLOWED_EXTENSIONS.split(","))):
                    file_path_list_to_allow.append(associated_file_path)
                elif sickchill.DELETE_NON_ASSOCIATED_FILES:
    ...
    def _combined_file_operation(self, file_path, new_path,  # pylint: disable=too-many-arguments, too-many-locals, too-many-branches
        ...
        file_list = [file_path]
        subfolders = ek(os.path.normpath, ek(os.path.dirname, file_path)) != ek(os.path.normpath, sickchill.TV_DOWNLOAD_DIR)
        if associated_files:
        ...
        for cur_file_path in file_list:
            ...
            if sickchill.SUBTITLES_DIR and cur_extension.endswith(tuple(SUBTITLE_EXTENSIONS)):
                subs_new_path = ek(os.path.join, new_path, sickchill.SUBTITLES_DIR)
    ...
    def _add_to_anidb_mylist(self, filePath):
        ...
        if helpers.set_up_anidb_connection():
            if not self.anidbEpisode:  # seems like we could parse the name before, now lets build the anidb object
                self.anidbEpisode = self._build_anidb_episode(sickchill.ADBA_CONNECTION, filePath)
    ...
    def _run_extra_scripts(self, ep_obj):
        ...
        if not sickchill.EXTRA_SCRIPTS:
            return

        file_path = self.file_path
        if isinstance(file_path, six.text_type):
            try:
                file_path = file_path.encode(sickchill.SYS_ENCODING)
        ...
        if isinstance(ep_location, six.text_type):
            try:
                ep_location = ep_location.encode(sickchill.SYS_ENCODING)
            except UnicodeEncodeError:
                # ignore it
                pass

        for curScriptName in sickchill.EXTRA_SCRIPTS:
            if isinstance(curScriptName, six.text_type):
                try:
                    curScriptName = curScriptName.encode(sickchill.SYS_ENCODING)
                except UnicodeEncodeError:
                    # ignore it
                    pass
    ...
    def process(self):  # pylint: disable=too-many-return-statements, too-many-locals, too-many-branches, too-many-statements
        ...
        if sickchill.USE_FREE_SPACE_CHECK:
        ...
        if not ek(os.path.isdir, ep_obj.show._location) and sickchill.CREATE_MISSING_SHOW_DIRS:  # pylint: disable=protected-access
        ...
        # add to anidb
        if ep_obj.show.is_anime and sickchill.ANIDB_USE_MYLIST:
        ...
        try:
            ...
            if self.process_method == METHOD_COPY:
                ...
                self._copy(self.file_path, dest_path, new_base_name, sickchill.MOVE_ASSOCIATED_FILES,
                           sickchill.USE_SUBTITLES and ep_obj.show.subtitles)
                ...
                self._move(self.file_path, dest_path, new_base_name, sickchill.MOVE_ASSOCIATED_FILES,
                           sickchill.USE_SUBTITLES and ep_obj.show.subtitles)
            elif self.process_method == METHOD_HARDLINK:
                self._hardlink(self.file_path, dest_path, new_base_name, sickchill.MOVE_ASSOCIATED_FILES,
                               sickchill.USE_SUBTITLES and ep_obj.show.subtitles)
            elif self.process_method == METHOD_SYMLINK:
                if helpers.is_file_locked(self.file_path, True):
                    raise EpisodePostProcessingFailedException("File is locked for reading/writing")
                self._moveAndSymlink(self.file_path, dest_path, new_base_name, sickchill.MOVE_ASSOCIATED_FILES,
                                     sickchill.USE_SUBTITLES and ep_obj.show.subtitles)
            elif self.process_method == METHOD_SYMLINK_REVERSED:
                self._symlink(self.file_path, dest_path, new_base_name, sickchill.MOVE_ASSOCIATED_FILES,
                                     sickchill.USE_SUBTITLES and ep_obj.show.subtitles)
            else:
        ...
        for cur_ep in [ep_obj] + ep_obj.relatedEps:
            with cur_ep.lock:
                ...
                # download subtitles
                if sickchill.USE_SUBTITLES and ep_obj.show.subtitles \
                    and (cur_ep.season != 0 or sickchill.SUBTITLES_INCLUDE_SPECIALS):
        ...
        if sickchill.USE_ICACLS and os.name == 'nt':
