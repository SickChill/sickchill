...
import sickchill
from sickchill import common, db, logger
...
class Notifier(object):
    ...
    def notify_snatch(self, ep_name):
        ep_name = ss(ep_name)
        if sickchill.PROWL_NOTIFY_ONSNATCH:
            ...
            if not recipients:
                ...
                for api in recipients:
                    self._send_prowl(prowl_api=api, prowl_priority=None, event=common.notifyStrings[common.NOTIFY_SNATCH],
                                     message=ep_name + " :: " + time.strftime(sickchill.DATE_PRESET + " " + sickchill.TIME_PRESET))

    def notify_download(self, ep_name):
        ep_name = ss(ep_name)
        if sickchill.PROWL_NOTIFY_ONDOWNLOAD:
            show = self._parse_episode(ep_name)
            recipients = self._generate_recipients(show)
            if not recipients:
                logger.log('Skipping prowl notify because there are no configured recipients', logger.DEBUG)
    ...
    def notify_subtitle_download(self, ep_name, lang):
        ep_name = ss(ep_name)
        if sickchill.PROWL_NOTIFY_ONSUBTITLEDOWNLOAD:
            show = self._parse_episode(ep_name)
            recipients = self._generate_recipients(show)
            if not recipients:
                logger.log('Skipping prowl notify because there are no configured recipients', logger.DEBUG)
                ...
                for api in recipients:
                    self._send_prowl(prowl_api=api, prowl_priority=None, event=common.notifyStrings[common.NOTIFY_SUBTITLE_DOWNLOAD],
                                     message=ep_name + " [" + lang + "] :: " + time.strftime(sickchill.DATE_PRESET + " " + sickchill.TIME_PRESET))

    def notify_git_update(self, new_version="??"):
        if sickchill.USE_PROWL:
    ...
    def _generate_recipients(show=None):
        ...
        if sickchill.PROWL_API:
            for api in sickchill.PROWL_API.split(','):
                if api.strip():
    ...
    def _send_prowl(prowl_api=None, prowl_priority=None, event=None, message=None, force=False):
        ...
        if not sickchill.USE_PROWL and not force:
            return False

        if prowl_api is None:
            prowl_api = sickchill.PROWL_API
            if len(prowl_api) == 0:
                return False

        if prowl_priority is None:
            prowl_priority = sickchill.PROWL_PRIORITY

        title = sickchill.PROWL_MESSAGE_TITLE
