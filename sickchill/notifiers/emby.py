...
class Notifier(object):
    ...
    def _notify_emby(self, message, host=None, emby_apikey=None):
        ...
        if not host:
            host = sickchill.EMBY_HOST
        if not emby_apikey:
            emby_apikey = sickchill.EMBY_APIKEY

        url = 'http://{0}/emby/Notifications/Admin'.format(host)
        values = {'Name': 'SickChill', 'Description': message, 'ImageUrl': sickchill.LOGO_URL}
...
##############################################################################
    ...
    def update_library(self, show=None):
        ...
        if sickchill.USE_EMBY:

            if not sickchill.EMBY_HOST:
            ...
            url = 'http://{0}/emby/Library/Series/Updated{1}'.format(sickchill.EMBY_HOST, query)
            values = {}
            data = urllib.parse.urlencode(values)
            try:
                req = urllib.request.Request(url, data)
                req.add_header('X-MediaBrowser-Token', sickchill.EMBY_APIKEY)
