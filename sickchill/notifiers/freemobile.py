...
class Notifier(object):
    ...
    def _sendFreeMobileSMS(self, title, msg, cust_id=None, apiKey=None):
        ...
        if cust_id is None:
            cust_id = sickchill.FREEMOBILE_ID
        if apiKey is None:
            apiKey = sickchill.FREEMOBILE_APIKEY
    ...
    def notify_snatch(self, ep_name, title=notifyStrings[NOTIFY_SNATCH]):
        if sickchill.FREEMOBILE_NOTIFY_ONSNATCH:
            self._notifyFreeMobile(title, ep_name)

    def notify_download(self, ep_name, title=notifyStrings[NOTIFY_DOWNLOAD]):
        if sickchill.FREEMOBILE_NOTIFY_ONDOWNLOAD:
            self._notifyFreeMobile(title, ep_name)

    def notify_subtitle_download(self, ep_name, lang, title=notifyStrings[NOTIFY_SUBTITLE_DOWNLOAD]):
        if sickchill.FREEMOBILE_NOTIFY_ONSUBTITLEDOWNLOAD:
            self._notifyFreeMobile(title, ep_name + ": " + lang)

    def notify_git_update(self, new_version="??"):
        if sickchill.USE_FREEMOBILE:
    ...
    def _notifyFreeMobile(self, title, message, cust_id=None, apiKey=None, force=False):  # pylint: disable=too-many-arguments
        ...
        if not sickchill.USE_FREEMOBILE and not force:
