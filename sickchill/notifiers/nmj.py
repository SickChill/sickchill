...
class Notifier(object):
    def notify_settings(self, host):
        ...
        if match:
            ...
            sickchill.NMJ_DATABASE = database
        else:
    ...
    def _notifyNMJ(self, host=None, database=None, mount=None, force=False):
        ...
        """
        if not sickchill.USE_NMJ and not force:
            logger.log("Notification for NMJ scan update not enabled, skipping this notification", logger.DEBUG)
            return False

        # fill in omitted parameters
        if not host:
            host = sickchill.NMJ_HOST
        if not database:
            database = sickchill.NMJ_DATABASE
        if not mount:
            mount = sickchill.NMJ_MOUNT
