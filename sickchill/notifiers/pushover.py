...
from sickchill.common import (NOTIFY_DOWNLOAD, NOTIFY_GIT_UPDATE, NOTIFY_GIT_UPDATE_TEXT, NOTIFY_LOGIN, NOTIFY_LOGIN_TEXT, NOTIFY_SNATCH,
...
class Notifier(object):
    ...
    def _sendPushover(self, msg, title, sound=None, userKey=None, apiKey=None, priority=None):
        ...
        if userKey is None:
            userKey = sickchill.PUSHOVER_USERKEY

        if apiKey is None:
            apiKey = sickchill.PUSHOVER_APIKEY

        if sound is None:
            sound = sickchill.PUSHOVER_SOUND

        if priority is None:
            priority = sickchill.PUSHOVER_PRIORITY
        ...
        try:
            if sickchill.PUSHOVER_SOUND != "default":
                args = {
                    "token": apiKey,
                    "user": userKey,
            ...
            if sickchill.PUSHOVER_DEVICE:
                args["device"] = sickchill.PUSHOVER_DEVICE
    ...
    def notify_snatch(self, ep_name, title=notifyStrings[NOTIFY_SNATCH]):
        if sickchill.PUSHOVER_NOTIFY_ONSNATCH:
            self._notifyPushover(title, ep_name)

    def notify_download(self, ep_name, title=notifyStrings[NOTIFY_DOWNLOAD]):
        if sickchill.PUSHOVER_NOTIFY_ONDOWNLOAD:
            self._notifyPushover(title, ep_name)

    def notify_subtitle_download(self, ep_name, lang, title=notifyStrings[NOTIFY_SUBTITLE_DOWNLOAD]):
        if sickchill.PUSHOVER_NOTIFY_ONSUBTITLEDOWNLOAD:
            self._notifyPushover(title, ep_name + ": " + lang)

    def notify_git_update(self, new_version="??"):
        if sickchill.USE_PUSHOVER:
    ...
    def _notifyPushover(self, title, message, sound=None, userKey=None, apiKey=None, force=False):
        ...
        if not sickchill.USE_PUSHOVER and not force:
