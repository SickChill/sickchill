...
class Notifier(object):
    ...
    @property
    def number(self):
        return self.client.phone_numbers.get(sickchill.TWILIO_PHONE_SID)

    @property
    def client(self):
        return twilio.rest.TwilioRestClient(sickchill.TWILIO_ACCOUNT_SID, sickchill.TWILIO_AUTH_TOKEN)

    def _notifyTwilio(self, message='', force=False, allow_raise=False):
        if not (sickchill.USE_TWILIO or force or self.number_regex.match(sickchill.TWILIO_TO_NUMBER)):
            return False
