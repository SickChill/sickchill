...
class Notifier(object):
    def __init__(self):
        self.session = sickchill.helpers.make_session()
    ...
    def _sendBoxcar2(self, msg, title, accesstoken):
        ...
        post_data = {
            ...
            'notification[icon_url]': sickchill.LOGO_URL
        }

        response = sickchill.helpers.getURL(self.url, post_data=post_data, session=self.session, timeout=60, returns='json')
        if not response:
