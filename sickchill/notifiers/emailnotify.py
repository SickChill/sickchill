...
class Notifier(object):
    ...
    def test_notify(self, host, port, smtp_from, use_tls, user, pwd, to):  # pylint: disable=too-many-arguments
        ...
        if sickchill.EMAIL_SUBJECT:
            msg[b'Subject'] = '[TEST] ' + sickchill.EMAIL_SUBJECT
        else:
    ...
    def notify_snatch(self, ep_name, title='Snatched:'):  # pylint: disable=unused-argument
        ...
        if sickchill.USE_EMAIL and sickchill.EMAIL_NOTIFY_ONSNATCH:
            ...
            if not to:
                ...
                if sickchill.EMAIL_SUBJECT:
                    msg[b'Subject'] = '[SN] ' + sickchill.EMAIL_SUBJECT
                else:
                    msg[b'Subject'] = 'Snatched: ' + ep_name
                msg[b'From'] = sickchill.EMAIL_FROM
                msg[b'To'] = ','.join(to)
                msg[b'Date'] = formatdate(localtime=True)
                if self._sendmail(sickchill.EMAIL_HOST, sickchill.EMAIL_PORT, sickchill.EMAIL_FROM, sickchill.EMAIL_TLS,
                                  sickchill.EMAIL_USER, sickchill.EMAIL_PASSWORD, to, msg):
    ...
    def notify_download(self, ep_name, title='Completed:'):  # pylint: disable=unused-argument
        ...
        if sickchill.USE_EMAIL and sickchill.EMAIL_NOTIFY_ONDOWNLOAD:
            show = self._parseEp(ep_name)
            to = self._generate_recipients(show)
            if not to:
                logger.log('Skipping email notify because there are no configured recipients', logger.DEBUG)
                ...
                if sickchill.EMAIL_SUBJECT:
                    msg[b'Subject'] = '[DL] ' + sickchill.EMAIL_SUBJECT
                else:
    ...
    def notify_subtitle_download(self, ep_name, lang, title='Downloaded subtitle:'):  # pylint: disable=unused-argument
        ...
        if sickchill.USE_EMAIL and sickchill.EMAIL_NOTIFY_ONSUBTITLEDOWNLOAD:
            show = self._parseEp(ep_name)
            to = self._generate_recipients(show)
            if not to:
                logger.log('Skipping email notify because there are no configured recipients', logger.DEBUG)
                ...
                if sickchill.EMAIL_SUBJECT:
                    msg[b'Subject'] = '[ST] ' + sickchill.EMAIL_SUBJECT
                else:
    ...
    def _generate_recipients(show):  # pylint: disable=too-many-branches
        ...
        if sickchill.EMAIL_LIST:
            for addr in sickchill.EMAIL_LIST.split(','):
                if len(addr.strip()) > 0:
                    addrs.append(addr)
