...
if hasattr('General', 'ignored_subs_list') and sickchill.IGNORED_SUBS_LIST:
    resultFilters.append("(" + sickchill.IGNORED_SUBS_LIST.replace(",", "|") + ")sub(bed|ed|s)?")
...
def filter_bad_releases(name, parse=True, show=None):
    ...
    elif sickchill.IGNORE_WORDS:
        ignore_words.extend(sickchill.IGNORE_WORDS.split(','))
    ...
    elif sickchill.REQUIRE_WORDS and not (show and show.rls_ignore_words):  # Only remove global require words from the list if we arent using show ignore words
        ignore_words = list(set(ignore_words).difference(x.strip() for x in sickchill.REQUIRE_WORDS.split(',') if x.strip()))
    ...
    elif sickchill.REQUIRE_WORDS:
        require_words.extend(sickchill.REQUIRE_WORDS.split(','))

    if show and show.rls_ignore_words:
        require_words = list(set(require_words).difference(x.strip() for x in show.rls_ignore_words.split(',') if x.strip()))
    elif sickchill.IGNORE_WORDS and not (show and show.rls_require_words):  # Only remove global ignore words from the list if we arent using show require words
        require_words = list(set(require_words).difference(x.strip() for x in sickchill.IGNORE_WORDS.split(',') if x.strip()))
