        ...
        if matches:
            ...
            if bestResult.is_air_by_date:
                ...
                if season_number is None or not episode_numbers:
                    try:
                        lINDEXER_API_PARMS = sickchill.indexerApi(bestResult.show.indexer).api_params.copy()

                        lINDEXER_API_PARMS['language'] = bestResult.show.lang or sickchill.INDEXER_DEFAULT_LANGUAGE

                        t = sickchill.indexerApi(bestResult.show.indexer).indexer(**lINDEXER_API_PARMS)
                    ...
                    except sickchill.indexer_episodenotfound:
                        ...
                        logger.log("Unable to contact " + sickchill.indexerApi(bestResult.show.indexer).name + ": " + ex(e), logger.WARNING)
                        episode_numbers = []
            ...
            # for sickchill, so we'd need to flag it.
