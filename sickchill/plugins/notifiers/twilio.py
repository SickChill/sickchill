...
@register
class TwilioNotifier(NotifierPlugin):
    id = "twilio"
    name = "Twilio"
    version = "1.0.0"
    legacy_sections: ClassVar[tuple[str, ...]] = ("Twilio",)
    schema: ClassVar[tuple[Field, ...]] = (
        Field(name="enabled", type="bool", default=False, legacy_keys=("use_twilio",)),
        Field(name="notify_onsnatch", type="bool", default=False, legacy_keys=("twilio_notify_onsnatch",)),
        Field(name="notify_ondownload", type="bool", default=False, legacy_keys=("twilio_notify_ondownload",)),
        Field(name="notify_onsubtitledownload", type="bool", default=False, legacy_keys=("twilio_notify_onsubtitledownload",)),
        Field(name="phone_sid", type="str", default="", legacy_keys=("twilio_phone_sid",)),
        Field(name="account_sid", type="str", default="", legacy_keys=("twilio_account_sid",)),
        Field(name="auth_token", type="str", default="", legacy_keys=("twilio_auth_token",)),
        Field(name="to_number", type="str", default="", legacy_keys=("twilio_to_number",)),
    )

    def _sync_settings(self) -> None:
        settings.USE_TWILIO = bool(self.ctx.get("enabled"))
        settings.TWILIO_NOTIFY_ONSNATCH = bool(self.ctx.get("notify_onsnatch"))
        settings.TWILIO_NOTIFY_ONDOWNLOAD = bool(self.ctx.get("notify_ondownload"))
        settings.TWILIO_NOTIFY_ONSUBTITLEDOWNLOAD = bool(self.ctx.get("notify_onsubtitledownload"))
        settings.TWILIO_PHONE_SID = self.ctx.get("phone_sid") or ""
        settings.TWILIO_ACCOUNT_SID = self.ctx.get("account_sid") or ""
        settings.TWILIO_AUTH_TOKEN = self.ctx.get("auth_token") or ""
        settings.TWILIO_TO_NUMBER = self.ctx.get("to_number") or ""

    def _impl(self):
        from sickchill.oldbeard.notifiers import twilio_notify as _mod
