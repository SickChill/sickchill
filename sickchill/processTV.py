...
def delete_folder(folder, check_empty=True):
    ...
    if sickchill.TV_DOWNLOAD_DIR and helpers.real_path(folder) == helpers.real_path(sickchill.TV_DOWNLOAD_DIR):
        return False
...
def process_dir(process_path, release_name=None, process_method=None, force=False, is_priority=None, delete_on=False, failed=False, mode="auto"):
    ...
    elif all([sickchill.TV_DOWNLOAD_DIR,
              ek(os.path.isdir, sickchill.TV_DOWNLOAD_DIR),
              ek(os.path.normpath, process_path) == ek(os.path.normpath, sickchill.TV_DOWNLOAD_DIR)]):
        process_path = ek(os.path.join, sickchill.TV_DOWNLOAD_DIR, ek(os.path.abspath, process_path).split(os.path.sep)[-1])
    ...
    process_method = process_method or sickchill.PROCESS_METHOD
    ...
    if release_name and (helpers.is_media_file(release_name) or helpers.is_rar_file(release_name)):
        ...
        generator_to_use = ek(os.walk, process_path, followlinks=sickchill.PROCESSOR_FOLLOW_SYMLINKS)
    ...
    # sickchill.DELRARCONTENTS allows to override even if method is NOT 'move'
    # manual post-processing will only delete when prompted by delete_on
    delete_rar_contents = any([sickchill.DELRARCONTENTS and mode != 'manual',
                               not sickchill.DELRARCONTENTS and mode == 'auto' and method_fallback == 'move',
...
def validate_dir(process_path, release_name, failed, result):  # pylint: disable=too-many-locals,too-many-branches,too-many-return-statements
    ...
    if sickchill.TV_DOWNLOAD_DIR and helpers.real_path(process_path) != helpers.real_path(sickchill.TV_DOWNLOAD_DIR) and helpers.is_hidden_folder(process_path):
    ...
    for current_directory, directory_names, file_names in ek(os.walk, process_path, topdown=False, followlinks=sickchill.PROCESSOR_FOLLOW_SYMLINKS):
        sync_files = filter(is_sync_file, file_names)
        if sync_files and sickchill.POSTPONE_IF_SYNC_FILES:
        ...
        if current_directory != sickchill.TV_DOWNLOAD_DIR and found_files:
...
def unrar(path, rar_files, force, result):  # pylint: disable=too-many-branches,too-many-statements
    ...
    if sickchill.UNPACK == 1 and rar_files:
        ...
        for archive in rar_files:
            ...
            try:
                ...
                if sickchill.UNPACK_DIR and os.path.isdir(sickchill.UNPACK_DIR): # verify the unpack dir exists
                    unpack_base_dir = sickchill.UNPACK_DIR
                else:
                    unpack_base_dir = path
                    if sickchill.UNPACK_DIR: # Let user know if we can't unpack there
...
def process_failed(process_path, release_name, result):
    ...
    if sickchill.USE_FAILED_DOWNLOADS:
        ...
        if sickchill.DELETE_FAILED and result.result:
