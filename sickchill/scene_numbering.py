...
def get_scene_numbering(indexer_id, indexer, season, episode, fallback_to_xem=True):
    ...
    showObj = Show.find(sickchill.showList, int(indexer_id))
...
def get_scene_absolute_numbering(indexer_id, indexer, absolute_number, fallback_to_xem=True):
    ...
    showObj = Show.find(sickchill.showList, indexer_id)
    if showObj and not showObj.is_scene:
        return absolute_number
...
def xem_refresh(indexer_id, indexer, force=False):
    ...
    if refresh or force:
        logger.log(
            'Looking up XEM scene mapping for show {0} on {1}'.format(indexer_id, sickchill.indexerApi(indexer).name),
            logger.DEBUG)
        ...
        try:
            # XEM MAP URL
            url = "http://thexem.de/map/havemap?origin={0}".format(sickchill.indexerApi(indexer).config['xem_origin'])
            parsed_json = sickchill.helpers.getURL(url, session=xem_session, returns='json')
            ...
            # XEM API URL
            url = "http://thexem.de/map/all?id={0}&origin={1}&destination=scene".format(indexer_id, sickchill.indexerApi(indexer).config['xem_origin'])

            parsed_json = sickchill.helpers.getURL(url, session=xem_session, returns='json')
            if not parsed_json or 'result' not in parsed_json or 'success' not in parsed_json['result']:
                logger.log('No XEM data for show "{0} on {1}"'.format(indexer_id, sickchill.indexerApi(indexer).name), logger.INFO)
                return
                ...
                if 'scene' in entry:
                    cl.append([
                        ...
                        [entry['scene']['season'], entry['scene']['episode'],
                         ...
                         entry[sickchill.indexerApi(indexer).config['xem_origin']]['season'],
                         entry[sickchill.indexerApi(indexer).config['xem_origin']]['episode']]
                    ])
                    cl.append([
                        "UPDATE tv_episodes SET absolute_number = ? WHERE showid = ? AND season = ? AND episode = ? AND absolute_number = 0",
                        [entry[sickchill.indexerApi(indexer).config['xem_origin']]['absolute'], indexer_id,
                         entry[sickchill.indexerApi(indexer).config['xem_origin']]['season'],
                         entry[sickchill.indexerApi(indexer).config['xem_origin']]['episode']]
                    ])
            ...
            logger.log(
                "Exception while refreshing XEM data for show " + str(indexer_id) + " on " + sickchill.indexerApi(
...
def fix_xem_numbering(indexer_id, indexer):  # pylint:disable=too-many-locals, too-many-branches, too-many-statements
    ...
    logger.log(
        'Fixing any XEM scene mapping issues for show {0} on {1}'.format(indexer_id, sickchill.indexerApi(indexer).name),
        logger.DEBUG)
