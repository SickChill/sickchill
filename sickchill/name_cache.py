...
def addNameToCache(name, indexer_id=0):
    ...
    name = sickchill.helpers.full_sanitizeSceneName(name)
...
def buildNameCache(show=None):
    ...
    """
    with nameCacheLock:
        sickchill.scene_exceptions.retrieve_exceptions()
    ...
    if not show:
        ...
        for curSeason in [-1] + sickchill.scene_exceptions.get_scene_seasons(show.indexerid):
            for name in set(sickchill.scene_exceptions.get_scene_exceptions(show.indexerid, season=curSeason) + [show.name]):
                name = sickchill.helpers.full_sanitizeSceneName(name)
                if name in nameCache:
                    continue
