...
import sickchill
from sickchill import helpers, logger, tvcache
...
class TorrentRssProvider(TorrentProvider):  # pylint: disable=too-many-instance-attributes
    ...
    def image_name(self):
        if ek(os.path.isfile, ek(os.path.join, sickchill.PROG_DIR, 'gui', sickchill.GUI_NAME, 'images', 'providers', self.get_id() + '.png')):
    ...
    @staticmethod
    def dumpHTML(data):
        dumpName = ek(os.path.join, sickchill.CACHE_DIR, 'custom_torrent.html')
