...
class BTNProvider(TorrentProvider):
        ...
        if self._check_auth_from_data(parsed_json):
            ...
            if 'results' in parsed_json and int(parsed_json['results']) >= results_per_page:
                ...
                for page in range(1, pages_needed + 1):
                    parsed_json = self._api_call(params, results_per_page, page * results_per_page)
    ...
    def _api_call(self, params=None, results_per_page=1000, offset=0):
        ...
        try:
            parsed_json = server.getTorrents(self.config('api_key'), params or {}, int(results_per_page), int(offset))
