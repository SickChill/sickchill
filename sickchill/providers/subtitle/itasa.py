...
class ItaSAProvider(Provider):
    ...
    def initialize(self):
        ...
        if self.username is not None and self.password is not None:
            ...
            params = {"username": self.username, "password": self.password, "apikey": self.apikey}
    ...
    def _download_zip(self, sub_id):
        ...
        params = {"authcode": self.auth_code, "apikey": self.apikey, "subtitle_id": sub_id}
    ...
    def _get_season_subtitles(self, show_id, season, sub_source):
        params = {"apikey": self.apikey, "show_id": show_id, "q": "Stagione %%%d" % season, "version": sub_source}
        ...
        for subtitle in root.findall("data/subtitles/subtitle"):
            if season_re.match(subtitle.find("name").text.lower()):
                logger.debug("Found season zip id %d - %r - %r", int(subtitle.find("id").text), subtitle.find("name").text, subtitle.find("version").text)
                ...
                with ZipFile(io.BytesIO(content)) as zf:
                    ...
                    for index, name in enumerate(zf.namelist()):
                        ...
                        if not match:  # pragma: no cover
                            ...
                            sub = ItaSASubtitle(
                                int(subtitle.find("id").text), subtitle.find("show_name").text, int(match.group(1)), int(match.group(2)), None, None, None, name
                            )
    ...
    def query(self, series, season, episode, source, resolution, country=None):
        ...
        logger.info("Getting the subtitle of show id %d, season %d episode %d, source %r", show_id, season, episode, source)
        subtitles = []
        ...
        params = {"apikey": self.apikey, "show_id": show_id, "q": "%dx%02d" % (season, episode), "version": sub_source}
        r = self.session.get(self.server_url + "subtitles/search", params=params, timeout=30)
        r.raise_for_status()
        root = ElementTree.fromstring(r.content)
        ...
        for subtitle in root.findall("data/subtitles/subtitle"):
            if "%dx%02d" % (season, episode) in subtitle.find("name").text.lower():
                ...
                sub = ItaSASubtitle(
                    int(subtitle.find("id").text), subtitle.find("show_name").text, season, episode, source, year, tvdb_id, subtitle.find("name").text
                )
