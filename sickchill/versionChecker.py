...
import sickchill
from sickchill import db, helpers, logger, notifiers, ui
...
class CheckVersion(object):
    ...
    def run(self, force=False):
        ...
        if self.updater:
            ...
            sickchill.BRANCH = self.get_branch()

            if self.check_for_new_version(force):
                if sickchill.AUTO_UPDATE:
                    ...
                    if self.run_backup_if_safe():
                        if sickchill.versionCheckScheduler.action.update():
    ...
    def _runbackup(self):
        ...
        try:
            backupDir = ek(os.path.join, sickchill.DATA_DIR, 'backup')
    ...
    def _backup(backupDir=None):
        if not backupDir:
            return False
        source = [
            ek(os.path.join, sickchill.DATA_DIR, 'sickchill.db'),
            sickchill.CONFIG_FILE,
        ...
        return helpers.backup_config_zip(source, target, sickchill.DATA_DIR)
    ...
    def safe_to_update(self):
        ...
        def postprocessor_safe():
            if not sickchill.autoPostProcessorScheduler.action.amActive:
        ...
        def showupdate_safe():
            if not sickchill.showUpdateScheduler.action.amActive:
    ...
    def getDBcompare(self):
        try:
            ...
            check_url = "http://raw.githubusercontent.com/{0}/{1}/{2}/sickchill/databases/mainDB.py".format(sickchill.GIT_ORG, sickchill.GIT_REPO, cur_hash)
    ...
    def find_install_type():
        ...
        if sickchill.BRANCH.startswith('build '):
            install_type = 'win'
        elif ek(os.path.isdir, ek(os.path.join, sickchill.PROG_DIR, '.git')):
            install_type = 'git'
        else:
            install_type = 'source'
    ...
    def check_for_new_version(self, force=False):
        ...
        if not self.updater or (not sickchill.VERSION_NOTIFY and not sickchill.AUTO_UPDATE and not force):
            logger.log("Version checking is disabled, not checking for the newest version")
            return False

        # checking for updates
        if not sickchill.AUTO_UPDATE:
    ...
    def check_for_new_news(self):
        ...
        try:
            news = helpers.getURL(sickchill.NEWS_URL, session=self.session, returns='text')
        except Exception:
        ...
        try:
            last_read = datetime.datetime.strptime(sickchill.NEWS_LAST_READ, '%Y-%m-%d')
        except Exception:
            last_read = 0
    ...
    def update(self):
        if self.updater:
            ...
            self.updater.branch = sickchill.BRANCH
...
class UpdateManager(object):  # pylint: disable=too-few-public-methods
    @staticmethod
    def get_update_url():
        return sickchill.WEB_ROOT + "/home/update/?pid=" + str(sickchill.PID)
...
class GitUpdateManager(UpdateManager):
    def __init__(self):
        ...
        self.branch = sickchill.BRANCH = self._find_installed_branch()
    ...
    def _find_working_git(self):
        ...
        if sickchill.GIT_PATH:
            main_git = '"' + sickchill.GIT_PATH + '"'
        else:
            main_git = 'git'
    ...
    def _run_git(git_path, args, log_errors=True):
        ...
        try:
            logger.log("Executing {0} with your shell in {1}".format(cmd, sickchill.PROG_DIR), logger.DEBUG)
            p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                 shell=True, cwd=sickchill.PROG_DIR)
    ...
    def _find_installed_version(self):
        ...
        if exit_status == 0 and output:
            ...
            sickchill.CUR_COMMIT_HASH = str(cur_commit_hash)
            return True
        else:
            return False
    ...
    def _check_github_for_update(self):
        ...
        output, errors_, exit_status = self._run_git(self._git_path, 'fetch {0} --prune'.format(sickchill.GIT_REMOTE))
        if exit_status != 0:
        ...
        output, stderr_, exit_status = self._run_git(self._git_path, 'branch --set-upstream-to {0}/{1}'.format(sickchill.GIT_REMOTE, self.branch), False)
        if exit_status != 0:
            self._run_git(self._git_path, 'branch -u {0}/{1}'.format(sickchill.GIT_REMOTE, self.branch), False)
    ...
    def set_newest_text(self):
        if self._num_commits_ahead:
            ...
            base_url = 'http://github.com/' + sickchill.GIT_ORG + '/' + sickchill.GIT_REPO
            if self._newest_commit_hash:
    ...
    def update(self):
        ...
        if sickchill.GIT_RESET:
            # self.clean() # This is removing user data and backups
            self.reset()

        if self.branch == self._find_installed_branch():
            stdout_, stderr_, exit_status = self._run_git(self._git_path, 'pull -f {0} {1}'.format(sickchill.GIT_REMOTE, self.branch))
        else:
        ...
        if exit_status == 0:
            ...
            notifiers.notify_git_update(sickchill.CUR_COMMIT_HASH or "")
            return True
        else:
            return False
    ...
    def list_remote_branches(self):
        # update remote origin url
        self.update_remote_origin()
        sickchill.BRANCH = self._find_installed_branch()

        branches, stderr_, exit_status = self._run_git(self._git_path, 'ls-remote --heads {0}'.format(sickchill.GIT_REMOTE))  # @UnusedVariable
    ...
    def update_remote_origin(self):
        if not sickchill.DEVELOPER:
            self._run_git(self._git_path, 'config remote.{0}.url {1}'.format(sickchill.GIT_REMOTE, sickchill.GIT_REMOTE_URL))


class SourceUpdateManager(UpdateManager):
    def __init__(self):

        self.branch = sickchill.BRANCH
        if sickchill.BRANCH == '':
            self.branch = self._find_installed_branch()

        self._cur_commit_hash = sickchill.CUR_COMMIT_HASH
        self._newest_commit_hash = None
        self._num_commits_behind = 0
    ...
    def _find_installed_branch():
        return sickchill.CUR_COMMIT_BRANCH if sickchill.CUR_COMMIT_BRANCH else "master"
    ...
    def _check_github_for_update(self):
        ...
        repo = sickchill.gh.get_organization(sickchill.GIT_ORG).get_repo(sickchill.GIT_REPO)
    ...
    def update(self):  # pylint: disable=too-many-statements
        ...
        tar_download_url = 'http://github.com/' + sickchill.GIT_ORG + '/' + sickchill.GIT_REPO + '/tarball/' + self.branch

        try:
            # prepare the update dir
            sr_update_dir = ek(os.path.join, sickchill.PROG_DIR, 'sr-update')
            ...
            logger.log("Moving files from " + content_dir + " to " + sickchill.PROG_DIR)
            for dirname, stderr_, filenames in ek(os.walk, content_dir):  # @UnusedVariable
                ...
                for curfile in filenames:
                    ...
                    new_path = ek(os.path.join, sickchill.PROG_DIR, dirname, curfile)

                    if ek(os.path.isfile, new_path):
                        ek(os.remove, new_path)
                    ek(os.renames, old_path, new_path)

            sickchill.CUR_COMMIT_HASH = self._newest_commit_hash
            sickchill.CUR_COMMIT_BRANCH = self.branch
