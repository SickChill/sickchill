...
opener.addheaders = [('User-agent', sickchill.sickbeard.common.USER_AGENT)]
...
def makeDir(path):
    ...
    if not os.path.isdir(path):
        try:
            ...
            sickchill.sickbeard.notifiers.synoindex_notifier.addFolder(path)
        except OSError:
            return False
    return True
        ...
        # not Windows, create all missing folders and set permissions
            ...
            for cur_folder in folder_list:
                ...
                try:
                    ...
                    # do the library update for synoindex
                    sickchill.sickbeard.notifiers.synoindex_notifier.addFolder(sofar)
                except (OSError, IOError) as error:
...
def rename_ep_file(cur_path, new_path, old_path_length=0):
    ...
    if cur_file_ext[1:] in SUBTITLE_EXTENSIONS:
        ...
        if sublang in sickchill.sickbeard.subtitles.subtitle_code_filter():
    ...
    while os.path.isdir(check_empty_dir) and check_empty_dir != keep_dir:
        ...
        if not check_files or (len(check_files) <= len(ignore_items) and all(
            ...
            try:
                ...
                # do the library update for synoindex
                sickchill.sickbeard.notifiers.synoindex_notifier.deleteFolder(check_empty_dir)
...
def _check_against_names(nameInQuestion, show, season=-1):
    ...
    showNames.extend(sickchill.sickbeard.scene_exceptions.get_scene_exceptions(show.indexerid, season=season))
...
def get_show(name, tryIndexers=False):
    ...
    try:
        ...
        cache = sickchill.sickbeard.name_cache.retrieveNameFromCache(name)
        if cache:
            fromCache = True
        ...
        if not showObj:
            scene_exceptions = sickchill.sickbeard.scene_exceptions.get_scene_exception_by_name_multiple(name)
        ...
        if showObj and not fromCache:
            sickchill.sickbeard.name_cache.addNameToCache(name, showObj.indexerid)
    except Exception as error:
