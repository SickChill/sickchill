...
def searchForNeededEpisodes():
    ...
    for curShow in show_list:
        if not curShow.paused:
            sickchill.sickbeard.name_cache.buildNameCache(curShow)
    ...
    providers = [x for x in sickchill.sickbeard.providers.sortedProviderList(settings.RANDOMIZE_PROVIDERS) if x.is_active and x.enable_daily and x.can_daily]
...
def searchProviders(show, episodes, manualSearch=False, downCurQuality=False):
    ...
    sickchill.sickbeard.name_cache.buildNameCache(show)

    origThreadName = threading.currentThread().name

    providers = [x for x in sickchill.sickbeard.providers.sortedProviderList(settings.RANDOMIZE_PROVIDERS) if x.is_active and x.can_backlog and x.enable_backlog]
    for curProvider in providers:
        threading.currentThread().name = origThreadName + " :: [" + curProvider.name + "]"
        curProvider.cache.update_cache()
