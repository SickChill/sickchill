...
class TVShow(object):
    ...
    def makeEpFromFile(self, filepath):
        ...
        for current_ep in episodes:
            ...
            elif not same_file and sickchill.sickbeard.helpers.is_media_file(filepath) and curEp.status not in Quality.DOWNLOADED + Quality.ARCHIVED + [IGNORED]:
    ...
    def deleteShow(self, full=False):
        ...
        for provider in sickchill.sickbeard.providers.__all__:
...
class TVEpisode(object):
    ...
    def loadFromDB(self, season, episode):
        ...
        if len(sql_results) > 1:
            ...
            sickchill.sickbeard.scene_numbering.xem_refresh(self.show.indexerid, self.show.indexer)
            ...
            if self.scene_absolute_number == 0:
                self.scene_absolute_number = sickchill.sickbeard.scene_numbering.get_scene_absolute_numbering(
                    self.show.indexerid,
                    self.show.indexer,
            ...
            if self.scene_season == 0 or self.scene_episode == 0:
                self.scene_season, self.scene_episode = sickchill.sickbeard.scene_numbering.get_scene_numbering(
                    self.show.indexerid,
                    self.show.indexer,
    ...
    def loadFromIndexer(self, season=None, episode=None):
        ...
        elif sickchill.sickbeard.helpers.is_media_file(self.location):
    ...
    def loadFromNFO(self, location):
        ...
        if not os.path.isdir(self.show._location):
            logger.info(str(self.show.indexerid) + ": The show dir is missing, not bothering to try loading the episode NFO")
            return

        logger.debug(str(self.show.indexerid) + ": Loading episode details from the NFO file associated with " + location)

        self.location = location

        if self.location != "":

            if self.status == UNKNOWN and sickchill.sickbeard.helpers.is_media_file(self.location):
