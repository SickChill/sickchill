...
from sickchill.sickbeard.providers import (abnormal, alpharatio, archetorrent, binsearch, bitcannon, bjshare, btn, cpasbien, danishbits, demonoid, elitetorrent,
                                           ...
                                           tokyotoshokan, torrent9, torrentbytes, torrentday, torrentleech, torrentproject, torrentz, tvchaosuk, xthor,
                                           yggtorrent)
...
def getProviderModule(name):
    name = name.lower()
    prefix = "sickchill.sickbeard.providers."
...
def check_enabled_providers():
    if not settings.DEVELOPER:
        ...
        if not (daily_enabled and backlog_enabled):
            ...
            sickchill.sickbeard.helpers.add_site_message(formatted_msg.format(searches=searches, web_root=settings.WEB_ROOT),
                                                         tag='no_providers_enabled', level='danger')
        else:
            sickchill.sickbeard.helpers.remove_site_message(tag='no_providers_enabled')
