...
import sickchill
from sickchill import classes, db, logger
from sickchill.common import USER_AGENT
...
urllib._urlopener = classes.SickChillURLopener()
...
def is_media_file(filename):
    ...
    try:
        ...
        return filname_parts[-1].lower() in MEDIA_EXTENSIONS or (sickchill.UNPACK == 2 and is_rar)
...
def makeDir(path):
    ...
    if not ek(os.path.isdir, path):
        try:
            ...
            sickchill.notifiers.synoindex_notifier.addFolder(path)
        except OSError:
            return False
    return True
...
def searchIndexerForShowID(regShowName, indexer=None, indexer_id=None, ui=None):
    ...
    for i in sickchill.indexerApi().indexers if not indexer else int(indexer or []):
        # Query Indexers for each search term and build the list of results
        lINDEXER_API_PARMS = sickchill.indexerApi(i).api_params.copy()
        if ui is not None:
            lINDEXER_API_PARMS['custom_ui'] = ui
        t = sickchill.indexerApi(i).indexer(**lINDEXER_API_PARMS)

        for name in showNames:
            logger.log(_("Trying to find {} on {}").format(name, sickchill.indexerApi(i).name), logger.DEBUG)
            ...
            if not (seriesname and series_id):
                continue
            ShowObj = Show.find(sickchill.showList, int(series_id))
        ...
        # not Windows, create all missing folders and set permissions
            ...
            for cur_folder in folder_list:
                ...
                try:
                    ...
                    # do the library update for synoindex
                    sickchill.notifiers.synoindex_notifier.addFolder(sofar)
                except (OSError, IOError) as error:
...
def rename_ep_file(cur_path, new_path, old_path_length=0):
    ...
    if cur_file_ext[1:] in SUBTITLE_EXTENSIONS:
        ...
        if sublang in sickchill.subtitles.subtitle_code_filter():
    ...
    while ek(os.path.isdir, check_empty_dir) and check_empty_dir != keep_dir:
        ...
        if not check_files or (len(check_files) <= len(ignore_items) and all(
            ...
            try:
                ...
                # do the library update for synoindex
                sickchill.notifiers.synoindex_notifier.deleteFolder(check_empty_dir)
...
def chmodAsParent(childPath):
    ...
    childPathStat = ek(os.stat, childPath.encode(sickchill.SYS_ENCODING))
...
def fixSetGroupID(childPath):
    ...
    if parentMode & stat.S_ISGID:
        ...
        childStat = ek(os.stat, childPath.encode(sickchill.SYS_ENCODING))
...
def update_anime_support():
    ...
    sickchill.ANIMESUPPORT = is_anime_in_show_list()
...
def get_all_episodes_from_absolute_number(show, absolute_numbers, indexer_id=None):
    ...
    if len(absolute_numbers):
        if not show and indexer_id:
            show = Show.find(sickchill.showList, indexer_id)
...
def anon_url(*url):
    ...
    """
    return '' if None in url else '{0}{1}'.format(sickchill.ANON_REDIRECT, ''.join(str(s) for s in url))
...
def encrypt(data, encryption_version=0, _decrypt=False):
    ...
    # Version 2: Simple XOR encryption (this is not very secure, but works)
        ...
        if _decrypt:
            return ''.join(chr(ord(x) ^ ord(y)) for (x, y) in izip(base64.decodestring(data), cycle(sickchill.ENCRYPTION_SECRET)))
        else:
            return base64.encodestring(
                ''.join(chr(ord(x) ^ ord(y)) for (x, y) in izip(data, cycle(sickchill.ENCRYPTION_SECRET)))).strip()
...
def _check_against_names(nameInQuestion, show, season=-1):
    ...
    showNames.extend(sickchill.scene_exceptions.get_scene_exceptions(show.indexerid, season=season))
...
def get_show(name, tryIndexers=False):
    ...
    try:
        ...
        cache = sickchill.name_cache.retrieveNameFromCache(name)
        if cache:
            fromCache = True
            showObj = Show.find(sickchill.showList, int(cache))

        # try indexers
        if not showObj and tryIndexers:
            showObj = Show.find(
                sickchill.showList, searchIndexerForShowID(full_sanitizeSceneName(name), ui=classes.ShowListUI)[2])

        # try scene exceptions
        if not showObj:
            ShowID = sickchill.scene_exceptions.get_scene_exception_by_name(name)[0]
            if ShowID:
                showObj = Show.find(sickchill.showList, int(ShowID))

        # add show to cache
        if showObj and not fromCache:
            sickchill.name_cache.addNameToCache(name, showObj.indexerid)
    except Exception as error:
...
def validateShow(show, season=None, episode=None):
    ...
    try:
        lINDEXER_API_PARMS = sickchill.indexerApi(show.indexer).api_params.copy()
        ...
        t = sickchill.indexerApi(show.indexer).indexer(**lINDEXER_API_PARMS)
...
def set_up_anidb_connection():
    ...
    if not sickchill.USE_ANIDB:
        logger.log(_("Usage of anidb disabled. Skiping"), logger.DEBUG)
        return False

    if not sickchill.ANIDB_USERNAME and not sickchill.ANIDB_PASSWORD:
        logger.log(_("anidb username and/or password are not set. Aborting anidb lookup."), logger.DEBUG)
        return False

    if not sickchill.ADBA_CONNECTION:
        def anidb_logger(msg):
            return logger.log(_("anidb: {0} ").format(msg), logger.DEBUG)

        try:
            sickchill.ADBA_CONNECTION = adba.Connection(keepAlive=True, log=anidb_logger)
        except Exception as error:
            logger.log(_("anidb exception msg: {0} ").format(error), logger.WARNING)
            return False

    try:
        if not sickchill.ADBA_CONNECTION.authed():
            sickchill.ADBA_CONNECTION.auth(sickchill.ANIDB_USERNAME, sickchill.ANIDB_PASSWORD)
        else:
            return True
    except Exception as error:
        logger.log(_("anidb exception msg: {0} ").format(error), logger.WARNING)
    ...
    return sickchill.ADBA_CONNECTION.authed()
        ...
        for indexer in sickchill.indexerApi().indexers:
            ...
            try:
                ...
                logger.log(_("Unable to map {} -> {} for show: {}, skipping it").format(
                    sickchill.indexerApi(showObj.indexer).name, sickchill.indexerApi(indexer).name, showObj.name), logger.DEBUG)
                continue
...
def request_defaults(kwargs):
    ...
    verify = certifi.where() if all([sickchill.SSL_VERIFY, kwargs.pop('verify', True)]) else False

    # request session proxies
    if allow_proxy and sickchill.PROXY_SETTING:
        logger.log(_("Using global proxy: {}").format(sickchill.PROXY_SETTING), logger.DEBUG)
        parsed_url = urlparse(sickchill.PROXY_SETTING)
        address = sickchill.PROXY_SETTING if parsed_url.scheme else 'http://' + sickchill.PROXY_SETTING
        proxies = {
            "http": address,
            "https": address,
        }
        ...
        for f in filenames:
            ...
            if ek(os.path.islink, fp) and not ek(os.path.isfile, fp):
                logger.log(_("Unable to get size for file {0} because the link to the file is not valid").format(fp),
                           logger.DEBUG if sickchill.IGNORE_BROKEN_SYMLINKS else logger.WARNING)
                continue
            try:
...
def verify_freespace(src, dest, oldfile=None, method="copy"):
    ...
    if not (ek(os.path.isdir, dest) or (sickchill.CREATE_MISSING_SHOW_DIRS and ek(os.path.isdir, ek(os.path.dirname, dest)))):
...
def get_showname_from_indexer(indexer, indexer_id, lang='en'):
    try:
        ...
        logger.log('{0}: {1!r}'.format(sickchill.indexerApi(indexer).name, lINDEXER_API_PARMS))
    ...
    except (sickchill.indexer_error, IOError) as e:
        logger.log(_("Show id {} not found on {}, not adding the show: {}").format(
            indexer_id, sickchill.indexerApi(indexer).name, ex(e)), logger.WARNING)
        return None
...
def add_site_message(message, tag=None, level='danger'):
    with sickchill.MESSAGES_LOCK:
        ...
        if tag:  # prevent duplicate messages of the same type
            ...
            existing = next((x for x, msg in six.iteritems(sickchill.SITE_MESSAGES) if msg.get('tag') == tag), None)
            if existing:
                sickchill.SITE_MESSAGES[existing] = to_add
                return

        global MESSAGE_COUNTER
        MESSAGE_COUNTER += 1
        sickchill.SITE_MESSAGES[MESSAGE_COUNTER] = to_add


def remove_site_message(key=None, tag=None):
    with sickchill.MESSAGES_LOCK:
        if key is not None and int(key) in sickchill.SITE_MESSAGES:
            del sickchill.SITE_MESSAGES[int(key)]
        elif tag is not None:
            found = [idx for idx, msg in six.iteritems(sickchill.SITE_MESSAGES) if msg.get('tag') == tag]
            for key in found:
                del sickchill.SITE_MESSAGES[key]


def sortable_name(name):
    if not sickchill.SORT_ARTICLE:
...
def manage_torrents_url(reset=False):
    if not reset:
        return sickchill.CLIENT_WEB_URLS.get('torrent', '')

    if not sickchill.USE_TORRENTS or not sickchill.TORRENT_HOST.lower().startswith('http') or sickchill.TORRENT_METHOD == 'blackhole' or \
            sickchill.ENABLE_HTTPS and not sickchill.TORRENT_HOST.lower().startswith('https'):
        sickchill.CLIENT_WEB_URLS['torrent'] = ''
        return sickchill.CLIENT_WEB_URLS.get('torrent')

    torrent_ui_url = re.sub('localhost|127.0.0.1', sickchill.LOCALHOST_IP or get_lan_ip(), sickchill.TORRENT_HOST or '', re.I)
    ...
    if sickchill.TORRENT_METHOD == 'utorrent':
        torrent_ui_url = '/'.join(s.strip('/') for s in (torrent_ui_url, 'gui/'))
    elif sickchill.TORRENT_METHOD == 'download_station':
        if test_exists(urljoin(torrent_ui_url, 'download/')):
            torrent_ui_url = urljoin(torrent_ui_url, 'download/')

    sickchill.CLIENT_WEB_URLS['torrent'] = ('', torrent_ui_url)[test_exists(torrent_ui_url)]
