...
import sickchill
from sickchill import common, db, logger, scheduler, search_queue, ui
...
class BacklogSearcher(object):
    def __init__(self):
        ...
        self.cycleTime = sickchill.BACKLOG_FREQUENCY / 60 / 24
    ...
    def searchBacklog(self, which_shows=None):
        ...
        if not (which_shows or curDate - self._lastBacklog >= self.cycleTime):
            logger.log("Running limited backlog on missed episodes " + str(sickchill.BACKLOG_DAYS) + " day(s) and older only")
            fromDate = datetime.date.today() - datetime.timedelta(days=sickchill.BACKLOG_DAYS)
        ...
        for curShow in show_list:
            ...
            for season, segment in six.iteritems(segments):
                ...
                sickchill.searchQueueScheduler.action.add_item(backlog_queue_item)  # @UndefinedVariable
