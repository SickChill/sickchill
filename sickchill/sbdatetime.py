...
import sickchill
from sickchill.network_timezones import sb_timezone
...
class sbdatetime(datetime.datetime):
    ...
    def convert_to_setting(self, dt=None):
        try:
            if sickchill.TIMEZONE_DISPLAY == 'local':
    ...
    def sbftime(self, dt=None, show_seconds=False, t_preset=None):
            ...
            if self is None:
                if dt is not None:
                    if t_preset is not None:
                        ...
                        strt = dt.strftime(sickchill.TIME_PRESET_W_SECONDS)
                    else:
                        strt = dt.strftime(sickchill.TIME_PRESET)
            else:
                if t_preset is not None:
                    strt = self.strftime(t_preset)
                elif show_seconds:
                    strt = self.strftime(sickchill.TIME_PRESET_W_SECONDS)
                else:
                    strt = self.strftime(sickchill.TIME_PRESET)
        finally:
            try:
                if sbdatetime.has_locale:
                    locale.setlocale(locale.LC_TIME, '')
        ...
        return strt.decode(sickchill.SYS_ENCODING)
    ...
    def sbfdate(self, dt=None, d_preset=None):
            ...
            if self is None:
                if dt is not None:
                    if d_preset is not None:
                        ...
                        strd = dt.strftime(sickchill.DATE_PRESET)
            else:
                if d_preset is not None:
                    strd = self.strftime(d_preset)
                else:
                    strd = self.strftime(sickchill.DATE_PRESET)
        finally:
        ...
        return strd.decode(sickchill.SYS_ENCODING)
            ...
            if self is None:
                if dt is not None:
                    ...
                    if t_preset is not None:
                        ...
                        strd += ', ' + dt.strftime(sickchill.TIME_PRESET_W_SECONDS)
                    else:
                        strd += ', ' + dt.strftime(sickchill.TIME_PRESET)
            else:
                if d_preset is not None:
                    strd = self.strftime(d_preset)
                else:
                ...
                if t_preset is not None:
                    ...
                    strd += ', ' + self.strftime(sickchill.TIME_PRESET_W_SECONDS)
                else:
                    strd += ', ' + self.strftime(sickchill.TIME_PRESET)
        finally:
            try:
                if sbdatetime.has_locale:
                    locale.setlocale(locale.LC_TIME, '')
