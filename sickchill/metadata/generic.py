...
class GenericMetadata(object):
    ...
    def _retrieve_show_image(self, image_type, show_obj, which=None):
        ...
        try:
            ...
            logger.log("Unable to look up show on " + sickchill.indexerApi(
                show_obj.indexer).name + ", not downloading images: " + ex(e), logger.WARNING)
            logger.log("{0} may be experiencing some problems. Try again later.".format(sickchill.indexerApi(show_obj.indexer).name), logger.DEBUG)
            return None

        if image_type not in ('fanart', 'poster', 'banner', 'poster_thumb', 'banner_thumb'):
            logger.log("Invalid image type " + str(image_type) + ", couldn't find it in the " + sickchill.indexerApi(
    ...
    def _season_posters_dict(self, show_obj, season):
        ...
        for seasonArtID in seasonsArtObj.keys():
            if int(seasonsArtObj[seasonArtID]['season']) == season and seasonsArtObj[seasonArtID]['language'] == (
                    indexer_lang or sickchill.INDEXER_DEFAULT_LANGUAGE):
    ...
    def _season_banners_dict(self, show_obj, season):
        ...
        # find the correct season in the TVDB object and just copy the dict into our result dict
        for seasonArtID in seasonsArtObj.keys():
            if int(seasonsArtObj[seasonArtID]['season']) == season and seasonsArtObj[seasonArtID]['language'] == (indexer_lang or sickchill.INDEXER_DEFAULT_LANGUAGE):
                result[season][seasonArtID] = seasonsArtObj[seasonArtID]['_bannerpath']
    ...
    def _retrieve_show_images_from_tmdb(self, show, img_type):
        ...
        tmdb = TMDB(sickchill.TMDB_API_KEY)
        ...
        try:
            ...
            if indexerid:
                request = fanartRequest(
                    apikey=sickchill.FANART_API_KEY,
                    id=indexerid,
