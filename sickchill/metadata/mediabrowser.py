...
class MediaBrowserMetadata(generic.GenericMetadata):
    ...
    def _show_data(self, show_obj):
        ...
        try:
            ...
            logger.log("Unable to find show with id " + str(show_obj.indexerid) + " on " + sickchill.indexerApi(
                show_obj.indexer).name + ", skipping it", logger.ERROR)
            raise

        except sickchill.indexer_error:
            logger.log(
                "" + sickchill.indexerApi(show_obj.indexer).name + " is down, can't use its data to make the NFO",
                logger.ERROR)
            raise

        # check for title and id
        if not (getattr(myShow, 'seriesname', None) and getattr(myShow, 'id', None)):
            logger.log("Incomplete info for show with id " + str(show_obj.indexerid) + " on " + sickchill.indexerApi(
        ...
        for curEpToWrite in eps_to_write:
            ...
            if curEpToWrite == ep_obj:
                ...
                try:
                    ...
                    Language.text = sickchill.INDEXER_DEFAULT_LANGUAGE  # tvrage api doesn't provide language so we must assume a value here
