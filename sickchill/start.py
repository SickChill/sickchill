    ...
    with settings.INIT_LOCK:
        ...
        for curProvider in providers.sortedProviderList():
            ...
            if hasattr(curProvider, 'enable_daily'):
                curProvider.enable_daily = curProvider.can_daily and check_setting_bool(settings.CFG, curProvider.get_id().upper(), curProvider.get_id(
                    '_enable_daily'), True)
        ...
        settings.showQueueScheduler = scheduler.Scheduler(
...
def save_config():
    ...
    new_config.update({
        ...
        'Cloudflare': {
            'auth_domain': settings.CF_AUTH_DOMAIN,
            'audience_policy': settings.CF_POLICY_AUD
        },
        ...
        'NMJ': {
            'use_nmj': int(settings.USE_NMJ),
            'nmj_host': settings.NMJ_HOST,
            'nmj_database': settings.NMJ_DATABASE,
            'nmj_mount': settings.NMJ_MOUNT
        },
        ...
        'TorrentRss': {
            'torrentrss_data': '!!!'.join([x.configStr() for x in settings.torrentRssProviderList])
        },
