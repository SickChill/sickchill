...
class TransmissionAPI(GenericClient):
    def __init__(self, host=None, username=None, password=None):
        ...
        self.url = '/'.join((self.host.rstrip('/'), sickchill.TORRENT_RPCURL.strip('/'), 'rpc'))
    ...
    def _get_auth(self):
        ...
        try:
            self.response = self.session.post(self.url, data=post_data.encode('utf-8'), timeout=120,
                                              verify=sickchill.TORRENT_VERIFY_CERT)
    ...
    def _add_torrent_uri(self, result):
        ...
        arguments = {
            'filename': result.url,
            'paused': int(sickchill.TORRENT_PAUSED)
        }

        if sickchill.TORRENT_PATH:
            arguments['download-dir'] = sickchill.TORRENT_PATH
    ...
    def _add_torrent_file(self, result):
        ...
        arguments = {
            ...
            'paused': 1 if sickchill.TORRENT_PAUSED else 0
        }
    ...
    def _set_torrent_seed_time(self, result):
        ...
        if sickchill.TORRENT_SEED_TIME and sickchill.TORRENT_SEED_TIME != -1:
            time = int(60 * float(sickchill.TORRENT_SEED_TIME))
            arguments = {'ids': [result.hash],
                         'seedIdleLimit': time,
                         'seedIdleMode': 1}
    ...
    def _set_torrent_priority(self, result):
        ...
        if result.priority == -1:
            ...
            if sickchill.TORRENT_HIGH_BANDWIDTH:
