...
from sickchill import ex, logger
...
class rTorrentAPI(GenericClient):  # pylint: disable=invalid-name
    ...
    def _get_auth(self):
        ...
        tp_kwargs = {}
        if sickchill.TORRENT_AUTH_TYPE != 'none':
            tp_kwargs['authtype'] = sickchill.TORRENT_AUTH_TYPE

        if not sickchill.TORRENT_VERIFY_CERT:
    ...
    def _add_torrent_uri(self, result):
        ...
        try:
            ...
            # Set label
            label = sickchill.TORRENT_LABEL
            if result.show.is_anime:
                label = sickchill.TORRENT_LABEL_ANIME
            if label:
                torrent.set_custom(1, label)

            if sickchill.TORRENT_PATH:
                torrent.set_directory(sickchill.TORRENT_PATH)

            if not sickchill.TORRENT_PAUSED:
                # Start torrent
                torrent.start()
    ...
    def _set_torrent_ratio(self, name):
        ...
        # ratio = int(float(sickchill.TORRENT_RATIO) * 100)
        #
        # try:
        # if ratio > 0:
        #
