...
class DownloadStationAPI(GenericClient):
            ...
            if error_code == 403:
                ...
                if destination and os.path.isabs(destination):
                    ...
                    if jdata.get('success'):
                        if destination == sickchill.SYNOLOGY_DSM_PATH:
                            sickchill.SYNOLOGY_DSM_PATH = data['destination']
                        elif destination == sickchill.TORRENT_PATH:
                            sickchill.TORRENT_PATH = data['destination']
    ...
    def _add_torrent_uri(self, result):
        ...
        params: :result: an object subclassing sickchill.classes.SearchResult
        """
        data = self._task_post_data
        data['uri'] = result.url

        if result.resultType == 'torrent':
            if sickchill.TORRENT_PATH:
                data['destination'] = sickchill.TORRENT_PATH
        elif sickchill.SYNOLOGY_DSM_PATH:
            data['destination'] = sickchill.SYNOLOGY_DSM_PATH
    ...
    def _add_torrent_file(self, result):
        ...
        if result.resultType == 'torrent':
            ...
            if sickchill.SYNOLOGY_DSM_PATH:
                data['destination'] = sickchill.SYNOLOGY_DSM_PATH
