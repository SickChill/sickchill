...
class qbittorrentAPI(GenericClient):
    ...
    def api(self):
        try:
            ...
            response = self.session.get(self.url, verify=sickchill.TORRENT_VERIFY_CERT)
    ...
    def _get_auth(self):
        ...
        if self.api > 1:
            ...
            try:
                self.response = self.session.get(self.host, verify=sickchill.TORRENT_VERIFY_CERT)
            except Exception:
                return None
    ...
    def _add_torrent_uri(self, result):
        ...
        if self.api > 6:
            if sickchill.TORRENT_PATH:
                data['savepath'] = sickchill.TORRENT_PATH
