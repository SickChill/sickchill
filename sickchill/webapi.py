...
def _get_root_dirs():
    ...
    root_dir["default_index"] = int(sickchill.ROOT_DIRS.split('|')[0])
...
class CMDComingEpisodes(ApiCall):
    ...
    def __init__(self, args, kwargs):
        ...
        self.paused, args = self.check_params(args, kwargs, "paused", bool(sickchill.COMING_EPS_DISPLAY_PAUSED), False,
                                              "bool", [])
...
class CMDEpisode(ApiCall):
    ...
    def run(self):
        ...
        show_obj = Show.find(sickchill.showList, int(self.indexerid))
        if not show_obj:
...
class CMDEpisodeSearch(ApiCall):
    ...
    def run(self):
        ...
        sickchill.searchQueueScheduler.action.add_item(ep_queue_item)  # @UndefinedVariable
...
class AbstractStartScheduler(ApiCall):
    ...
    def scheduler_class_str(self):
        return 'sickchill.scheduler.Scheduler'

    def run(self):
        error_str = 'Start scheduler failed'
        if not isinstance(self.scheduler, sickchill.scheduler.Scheduler):
...
class CMDFullSubtitleSearch(AbstractStartScheduler):
    ...
    @property
    def scheduler(self):
        return sickchill.subtitlesFinderScheduler
...
class CMDProperSearch(AbstractStartScheduler):
    ...
    @property
    def scheduler(self):
        return sickchill.properFinderScheduler
...
class CMDDailySearch(AbstractStartScheduler):
    ...
    @property
    def scheduler(self):
        return sickchill.dailySearchScheduler
    ...
    def run(self):
        ...
        for ep_obj in ep_list:
            with ep_obj.lock:
                ...
                if self.status == FAILED and not sickchill.USE_FAILED_DOWNLOADS:
        ...
        if start_backlog:
            ...
            for season, segment in six.iteritems(segments):
                ...
                sickchill.searchQueueScheduler.action.add_item(cur_backlog_queue_item)  # @UndefinedVariable
...
class CMDSubtitleSearch(ApiCall):
    ...
    def run(self):
        ...
        if new_subtitles:
            new_languages = [sickchill.subtitles.name_from_code(code) for code in new_subtitles]
...
class CMDPostProcess(ApiCall):
    ...
    def run(self):
        ...
        if not self.path and not sickchill.TV_DOWNLOAD_DIR:
            return _responds(RESULT_FAILURE, msg="You need to provide a path or set TV Download Dir")

        if not self.path:
            self.path = sickchill.TV_DOWNLOAD_DIR

        if not self.type:
            self.type = 'manual'

        data = sickchill.postProcessorTaskScheduler.action.add_item(
...
class CMDSickChill(ApiCall):
    ...
    def run(self):
        ...
        data = {"sr_version": sickchill.BRANCH, "api_version": self.version,
...
class CMDSickChillAddRootDir(ApiCall):
    ...
    def run(self):
        ...
        if sickchill.ROOT_DIRS == "":
            ...
            root_dirs = sickchill.ROOT_DIRS.split('|')
            index = int(sickchill.ROOT_DIRS.split('|')[0])
            root_dirs.pop(0)
            # clean up the list - replace %xx escapes by their single-character equivalent
            root_dirs = [urllib.parse.unquote_plus(x) for x in root_dirs]
            for x in root_dirs:
        ...
        sickchill.ROOT_DIRS = root_dirs_new
...
class CMDSickChillCheckScheduler(ApiCall):
    ...
    def run(self):
        ...
        backlog_paused = sickchill.searchQueueScheduler.action.is_backlog_paused()  # @UndefinedVariable
        backlog_running = sickchill.searchQueueScheduler.action.is_backlog_in_progress()  # @UndefinedVariable
        next_backlog = sickchill.backlogSearchScheduler.nextRun().strftime(dateFormat).decode(sickchill.SYS_ENCODING)
...
class CMDSickChillGetDefaults(ApiCall):
    ...
    def run(self):
        ...
        any_qualities, best_qualities = _map_quality(sickchill.QUALITY_DEFAULT)

        data = {"status": statusStrings[sickchill.STATUS_DEFAULT].lower(),
                "flatten_folders": int(not sickchill.SEASON_FOLDERS_DEFAULT),
                "season_folders": int(sickchill.SEASON_FOLDERS_DEFAULT),
                "initial": any_qualities, "archive": best_qualities,
                "future_show_paused": int(sickchill.COMING_EPS_DISPLAY_PAUSED)}
        return _responds(RESULT_SUCCESS, data)
...
class CMDSickChillPauseBacklog(ApiCall):
    ...
    def run(self):
        ...
        if self.pause:
            sickchill.searchQueueScheduler.action.pause_backlog()  # @UndefinedVariable
            return _responds(RESULT_SUCCESS, msg="Backlog paused")
        else:
            sickchill.searchQueueScheduler.action.unpause_backlog()  # @UndefinedVariable
...
class CMDSickChillPing(ApiCall):
    ...
    def run(self):
        ...
        if sickchill.started:
            return _responds(RESULT_SUCCESS, {"pid": sickchill.PID}, "Pong")
        else:
...
class CMDSickChillRestart(ApiCall):
    ...
    def run(self):
        """ Restart SickChill """
        if not Restart.restart(sickchill.PID):
...
class CMDSickChillSearchIndexers(ApiCall):
    ...
    def __init__(self, args, kwargs):
        ...
        self.valid_languages = sickchill.indexerApi().config['langabbv_to_id']
        self.name, args = self.check_params(args, kwargs, "name", None, False, "string", [])
        self.lang, args = self.check_params(args, kwargs, "lang", sickchill.INDEXER_DEFAULT_LANGUAGE, False, "string",
    ...
    def run(self):
        ...
        if self.name and not self.indexerid:  # only name was given
            for _indexer in sickchill.indexerApi().indexers if self.indexer == 0 else [int(self.indexer)]:
                indexer_api_params = sickchill.indexerApi(_indexer).api_params.copy()

                indexer_api_params['language'] = self.lang or sickchill.INDEXER_DEFAULT_LANGUAGE

                indexer_api_params['actors'] = False
                indexer_api_params['custom_ui'] = classes.AllShowsListUI

                t = sickchill.indexerApi(_indexer).indexer(**indexer_api_params)

                try:
                    api_data = t[str(self.name).encode()]
                except (sickchill.indexer_shownotfound, sickchill.indexer_showincomplete, sickchill.indexer_error):
...
class CMDSickChillSetDefaults(ApiCall):
    ...
    def __init__(self, args, kwargs):
        ...
        self.season_folders, args = self.check_params(args, kwargs, "flatten_folders", not bool(sickchill.SEASON_FOLDERS_DEFAULT), False, "bool", [])
    ...
    def run(self):
        ...
        if i_quality_id or a_quality_id:
            sickchill.QUALITY_DEFAULT = Quality.combineQualities(i_quality_id, a_quality_id)
        ...
        if self.status:
            ...
            sickchill.STATUS_DEFAULT = self.status

        if self.season_folders is not None:
            sickchill.SEASON_FOLDERS_DEFAULT = int(self.season_folders)

        if self.future_show_paused is not None:
            sickchill.COMING_EPS_DISPLAY_PAUSED = int(self.future_show_paused)
...
class CMDSickChillShutdown(ApiCall):
    ...
    def run(self):
        """ Shutdown SickChill """
        if not Shutdown.stop(sickchill.PID):
...
class CMDShowAddExisting(ApiCall):
    ...
    def __init__(self, args, kwargs):
        ...
        self.season_folders, args = self.check_params(args, kwargs, "flatten_folders",
                                                      bool(sickchill.SEASON_FOLDERS_DEFAULT), False, "bool", [])
        self.season_folders, args = self.check_params(args, kwargs, "season_folders",
                                                      self.season_folders, False, "bool", [])
        self.subtitles, args = self.check_params(args, kwargs, "subtitles", int(sickchill.USE_SUBTITLES),
                                                 False, "int", [])
    ...
    def run(self):
        ...
        new_quality = int(sickchill.QUALITY_DEFAULT)
        i_quality_id = []
        a_quality_id = []
        ...
        sickchill.showQueueScheduler.action.add_show(
            ...
            default_status=sickchill.STATUS_DEFAULT, quality=new_quality,
...
class CMDShowAddNew(ApiCall):
    ...
    def __init__(self, args, kwargs):
        ...
        self.lang, args = self.check_params(args, kwargs, "lang", sickchill.INDEXER_DEFAULT_LANGUAGE, False, "string",
                                            self.valid_languages.keys())
        self.subtitles, args = self.check_params(args, kwargs, "subtitles", bool(sickchill.USE_SUBTITLES),
                                                 False, "bool", [])
        self.anime, args = self.check_params(args, kwargs, "anime", bool(sickchill.ANIME_DEFAULT), False,
                                             "bool", [])
        self.scene, args = self.check_params(args, kwargs, "scene", bool(sickchill.SCENE_DEFAULT), False,
                                             "bool", [])
    ...
    def run(self):
        ...
        new_status = sickchill.STATUS_DEFAULT
        if self.status:
            # convert the string status to a int
            for status in statusStrings:
                if statusStrings[status].lower() == str(self.status).lower():
        ...
        # use default status as a fail-safe
        default_ep_status_after = sickchill.STATUS_DEFAULT_AFTER
...
class CMDShowSeasons(ApiCall):
    ...
    def run(self):
        ...
        sho_obj = Show.find(sickchill.showList, int(self.indexerid))
        if not sho_obj:
            return _responds(RESULT_FAILURE, msg="Show not found")
...
class CMDShowUpdate(ApiCall):
    ...
    def run(self):
        ...
        try:
            sickchill.showQueueScheduler.action.update_show(show_obj, True)  # @UndefinedVariable
