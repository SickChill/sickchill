...
def _downloadResult(result):
    ...
    # if it's an nzb data result
        ...
        file_name = ek(os.path.join, sickchill.NZB_DIR, result.name + ".nzb")
...
def snatchEpisode(result, endStatus=SNATCHED):  # pylint: disable=too-many-branches, too-many-statements
    ...
    if sickchill.ALLOW_HIGH_PRIORITY:
    ...
    if result.resultType in (GenericProvider.NZB, GenericProvider.NZBDATA):
        if sickchill.NZB_METHOD == "blackhole":
            dlResult = _downloadResult(result)
        elif sickchill.NZB_METHOD == "sabnzbd":
            dlResult = sab.sendNZB(result)
        elif sickchill.NZB_METHOD == "nzbget":
            is_proper = True if endStatus == SNATCHED_PROPER else False
            dlResult = nzbget.sendNZB(result, is_proper)
        elif sickchill.NZB_METHOD == "download_station":
            client = clients.getClientInstance(sickchill.NZB_METHOD)(
                sickchill.SYNOLOGY_DSM_HOST, sickchill.SYNOLOGY_DSM_USERNAME, sickchill.SYNOLOGY_DSM_PASSWORD)
            dlResult = client.sendNZB(result)
        else:
            logger.log("Unknown NZB action specified in config: " + sickchill.NZB_METHOD, logger.ERROR)
            dlResult = False

    # Torrents can be sent to clients or saved to disk
    elif result.resultType == GenericProvider.TORRENT:
        # torrents are saved to disk when blackhole mode
        if sickchill.TORRENT_METHOD == "blackhole":
            dlResult = _downloadResult(result)
        else:
            ...
            if result.content or result.url.startswith('magnet'):
                client = clients.getClientInstance(sickchill.TORRENT_METHOD)()
...
def pickBestResult(results, show):  # pylint: disable=too-many-branches
    ...
    for cur_result in results:
        ...
        if hasattr(cur_result, 'size'):
            if sickchill.USE_FAILED_DOWNLOADS and failed_history.hasFailed(cur_result.name, cur_result.size,
...
def searchForNeededEpisodes():
    ...
    show_list = sickchill.showList
    fromDate = datetime.date.fromordinal(1)
    episodes = []

    for curShow in show_list:
        if not curShow.paused:
            sickchill.name_cache.buildNameCache(curShow)
    ...
    providers = [x for x in sickchill.providers.sortedProviderList(sickchill.RANDOMIZE_PROVIDERS) if x.is_active and x.enable_daily and x.can_daily]
...
def searchProviders(show, episodes, manualSearch=False, downCurQuality=False):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
    ...
    sickchill.name_cache.buildNameCache(show)

    origThreadName = threading.currentThread().name

    providers = [x for x in sickchill.providers.sortedProviderList(sickchill.RANDOMIZE_PROVIDERS) if x.is_active and x.can_backlog and x.enable_backlog]
    for curProvider in providers:
        threading.currentThread().name = origThreadName + " :: [" + curProvider.name + "]"
        curProvider.cache.update_cache()
