...
# Resolutions
resolution = [
    re.compile(r"(?P<vres>4320|2160|1080|720|480|360)(?P<scan>[pi])", re.IGNORECASE),
    re.compile(r"(:?[sph]d)(:?.?tv)?[ \-_\.]*(?P<vres>4320|2160|1080|720|480|360)(:?(?P<scan>[pi])|[^\d]|$)", re.IGNORECASE),
]

# Sources
tv = re.compile(r"([sph]d).?tv|tv(rip|mux)", re.IGNORECASE)
dvd = re.compile(r"(?P<hd>hd)?dvd(?P<rip>rip|mux)?", re.IGNORECASE)
web = re.compile(r"(web(?P<type>rip|mux|hd|uhd|.?dl|\b))", re.IGNORECASE)
...
hevc = re.compile(r"\.hevc\.", re.IGNORECASE)
trueHD = re.compile(r"\.TrueHD\.")

# Codecs
avc = re.compile(r"([xh].?26[45]|AVC)", re.IGNORECASE)
