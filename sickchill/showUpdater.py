...
class ShowUpdater(object):  # pylint: disable=too-few-public-methods
    ...
    def run(self, force=False):  # pylint: disable=unused-argument, too-many-locals, too-many-branches, too-many-statements
        ...
        if not data:
            logger.log('Could not get the recently updated show data from {0}. Retrying later. Url was: {1}'.format(sickchill.indexerApi(INDEXER_TVDB).name, url))
            self.amActive = False
            return
        ...
        pi_list = []
        for cur_show in sickchill.showList:
            ...
            try:
                ...
                if sickchill.indexerApi(cur_show.indexer).name == 'theTVDB':
                    if cur_show.indexerid in updated_shows:
                        pi_list.append(sickchill.showQueueScheduler.action.update_show(cur_show, True))
                    else:
                        pi_list.append(sickchill.showQueueScheduler.action.refresh_show(cur_show, False))
