...
def sendNZB(nzb):  # pylint:disable=too-many-return-statements, too-many-branches, too-many-statements
    ...
    category = sickchill.SAB_CATEGORY
    if nzb.show.is_anime:
        category = sickchill.SAB_CATEGORY_ANIME

    # if it aired more than 7 days ago, override with the backlog category IDs
    for curEp in nzb.episodes:
        if datetime.date.today() - curEp.airdate > datetime.timedelta(days=7):
            category = sickchill.SAB_CATEGORY_ANIME_BACKLOG if nzb.show.is_anime else sickchill.SAB_CATEGORY_BACKLOG

    # set up a dict with the URL params in it
    params = {'output': 'json'}
    if sickchill.SAB_USERNAME:
        params['ma_username'] = sickchill.SAB_USERNAME
    if sickchill.SAB_PASSWORD:
        params['ma_password'] = sickchill.SAB_PASSWORD
    if sickchill.SAB_APIKEY:
        params['apikey'] = sickchill.SAB_APIKEY

    if category:
        params['cat'] = category

    if nzb.priority:
        params['priority'] = 2 if sickchill.SAB_FORCED else 1

    logger.log('Sending NZB to SABnzbd')
    url = urljoin(sickchill.SAB_HOST, 'api')
