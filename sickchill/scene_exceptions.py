...
def get_scene_exception_by_name_multiple(show_name):
    ...
    for cur_exception in all_exception_results:
        ...
        if show_name.lower() in (
                ...
                sickchill.helpers.sanitizeSceneName(cur_exception_name).lower().replace('.', ' ')):
...
def retrieve_exceptions():  # pylint:disable=too-many-locals, too-many-branches
    ...
    for indexer in sickchill.indexerApi().indexers:
        if shouldRefresh(sickchill.indexerApi(indexer).name):
            do_refresh = True

    if do_refresh:
        loc = sickchill.indexerApi(INDEXER_TVDB).config['scene_loc']
        logger.log("Checking for scene exception updates from {0}".format(loc))

        session = sickchill.indexerApi(INDEXER_TVDB).session
        proxy = sickchill.PROXY_SETTING
        if proxy and sickchill.PROXY_INDEXERS:
            session.proxies = {
                "http": proxy,
                "https": proxy,
            }
        ...
        if not jdata:
            ...
            for indexer in sickchill.indexerApi().indexers:
                try:
                    setLastRefresh(sickchill.indexerApi(indexer).name)
                    for indexer_id in jdata[sickchill.indexerApi(indexer).config['xem_origin']]:
                        alias_list = [
                            {scene_exception: int(scene_season)}
                            for scene_season in jdata[sickchill.indexerApi(indexer).config['xem_origin']][indexer_id]
                            for scene_exception in jdata[sickchill.indexerApi(indexer).config['xem_origin']][indexer_id][scene_season]
                        ]
    ...
    if shouldRefresh('xem'):
        for indexer in sickchill.indexerApi().indexers:
            logger.log("Checking for XEM scene exception updates for {0}".format
                       (sickchill.indexerApi(indexer).name))

            url = "http://thexem.de/map/allNames?origin={0}&seasonNumbers=1".format(sickchill.indexerApi(indexer).config['xem_origin'])
            ...
            if not parsed_json:
                logger.log("Check scene exceptions update failed for {0}, Unable to get URL: {1}".format
                           (sickchill.indexerApi(indexer).name, url), logger.DEBUG)
                continue
