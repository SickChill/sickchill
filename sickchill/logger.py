...
class Logger(object):  # pylint: disable=too-many-instance-attributes
    ...
    def init_logging(self, console_logging=False, file_logging=False, debug_logging=False, database_logging=False):
        ...
        """
        self.log_file = self.log_file or ek(os.path.join, sickchill.LOG_DIR, 'sickchill.log')
        ...
        if self.file_logging:
            rfh = logging.handlers.RotatingFileHandler(
                self.log_file, maxBytes=int(sickchill.LOG_SIZE * 1048576), backupCount=sickchill.LOG_NR, encoding='utf-8'
            )
    ...
    def set_level(self):
        self.debug_logging = sickchill.DEBUG
        self.database_logging = sickchill.DBDEBUG
    ...
    def log(self, msg, level=INFO, *args, **kwargs):
        ...
        cur_hash = ''
        if level == ERROR and sickchill.CUR_COMMIT_HASH and len(sickchill.CUR_COMMIT_HASH) > 6:
            cur_hash = '[{0}] '.format(
                sickchill.CUR_COMMIT_HASH[:7]
            )
    ...
    def log_error_and_exit(self, error_msg, *args, **kwargs):
        ...
        if not self.console_logging:
            sys.exit(error_msg.encode(sickchill.SYS_ENCODING, 'xmlcharrefreplace'))
        else:
            sys.exit(1)
    ...
    def submit_errors(self):  # pylint: disable=too-many-branches,too-many-locals
        ...
        gh_credentials = (sickchill.GIT_AUTH_TYPE == 0 and sickchill.GIT_USERNAME and sickchill.GIT_PASSWORD) \
            or (sickchill.GIT_AUTH_TYPE == 1 and sickchill.GIT_TOKEN)

        if not all((gh_credentials, sickchill.DEBUG, sickchill.gh, classes.ErrorViewer.errors)):
        ...
        try:
            ...
            for i in range(1, int(sickchill.LOG_NR)):
            ...
            for cur_error in sorted(classes.ErrorViewer.errors, key=lambda error: error.time, reverse=True)[:500]:
                ...
                for i, data in enumerate(__log_data):
                    ...
                    if match:
                        ...
                        if LOGGING_LEVELS[level] == ERROR:
                            ...
                            if paste_data:
                                gist = sickchill.gh.get_user().create_gist(False, {'sickchill.log': InputFileContent(paste_data)})
                            break
                    else:
                        gist = 'No ERROR found'
                ...
                msg = [
                    ...
                    'Branch: **{0}**'.format(sickchill.BRANCH),
                    'Commit: SickChill/SickChill@{0}'.format(sickchill.CUR_COMMIT_HASH),
                    log_link,
                    '### ERROR',
                    '```',
                    cur_error.message,
...
def log_data(min_level, log_filter, log_search, max_lines):
    ...
    for x in data:
        ...
        if match:
            ...
            if not sickchill.DEBUG and level == 'DEBUG':
                continue

            if not sickchill.DBDEBUG and level == 'DB':
                continue
