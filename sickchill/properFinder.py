...
class ProperFinder(object):  # pylint: disable=too-few-public-methods
    ...
    def run(self, force=False):  # pylint: disable=unused-argument
        ...
        run_at = ""
        if None is sickchill.properFinderScheduler.start_time:
            run_in = sickchill.properFinderScheduler.lastRun + sickchill.properFinderScheduler.cycleTime - datetime.datetime.now()
    ...
    def _getProperList(self):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        providers = [x for x in sickchill.providers.sortedProviderList(sickchill.RANDOMIZE_PROVIDERS) if x.is_active]
