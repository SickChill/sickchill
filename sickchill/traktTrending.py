...
class traktTrending(object):
    ...
    def fetch_trending_shows(self, trakt_list, page_url):
        ...
        try:
            not_liked_show = ""
            if sickchill.TRAKT_ACCESS_TOKEN != '':
                library_shows = trakt_api.traktRequest("sync/collection/shows?extended=full") or []
                if sickchill.TRAKT_BLACKLIST_NAME:
                    not_liked_show = trakt_api.traktRequest("users/" + sickchill.TRAKT_USERNAME + "/lists/" + sickchill.TRAKT_BLACKLIST_NAME + "/items") or []
                else:
    ...
    def get_image_url(indexer_id):
        ...
        try:
            lINDEXER_API_PARMS = sickchill.indexerApi(INDEXER_TVDB).api_params.copy()

            lINDEXER_API_PARMS['banners'] = True

            t = sickchill.indexerApi(INDEXER_TVDB).indexer(**lINDEXER_API_PARMS)
            indexer_show_obj = t[int(indexer_id)]
        except (sickchill.indexer_error, IOError) as e:
            logger.log("Show id " + indexer_id + " not found on " + sickchill.indexerApi(INDEXER_TVDB).name +
    ...
    def get_image_path(image_name):
        path = ek(os.path.abspath, ek(os.path.join, sickchill.CACHE_DIR, 'images', 'trakt_trending'))
