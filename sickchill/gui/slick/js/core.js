...
var SICKCHILL = {
    common: {
        init: function() {
            (function() {
                ...
                if (metaToBool('sickchill.SICKCHILL_BACKGROUND')) {
                    $.backstretch(srRoot + '/ui/sickchill_background');
                    $('.backstretch').css('opacity', getMeta('sickchill.FANART_BACKGROUND_OPACITY')).fadeIn('500');
                }
            })();
            ...
            if (metaToBool('sickchill.FUZZY_DATING')) {
        ...
        index: function() {
            ...
            $('.show-grid').imagesLoaded(function() {
                ...
                $('.show-grid').show().isotope({
                    ...
                    sortBy: getMeta('sickchill.POSTER_SORTBY'),
                    sortAscending: getMeta('sickchill.POSTER_SORTDIR'),
                    layoutMode: 'masonry',
                    masonry: {
                        isFitWidth: true
                    },
                    getSortData: {
                        name: function(itemElem) {
                            ...
                            return (metaToBool('sickchill.SORT_ARTICLE') ? name : name.replace(/^((?:The|A|An)\s)/i, '')).toLowerCase();
                        },
            ...
            $('#popover').popover({
                ...
                if (metaToBool('sickchill.ANIME_SPLIT_HOME')) {
                    $.tablesorter.columnSelector.attachTo($('#showListTableAnime'), '#popover-target');
                }
            });
        },
        displayShow: function() {
            if (metaToBool('sickchill.FANART_BACKGROUND')) {
        ...
        index: function() {
            $('#historyTable:has(tbody tr)').tablesorter({
                ...
                textExtraction: (function() {
                    if (isMeta('sickchill.HISTORY_LAYOUT', ['detailed'])) {
                        return {
                    ...
                    if (isMeta('sickchill.USE_SUBTITLES', ['True'])) {
    ...
    schedule: {
        ...
        index: function() {
            if (isMeta('sickchill.COMING_EPS_LAYOUT', ['list'])) {
                const sortCodes = {date: 0, show: 2, network: 5};
                const sort = getMeta('sickchill.COMING_EPS_SORT');
            ...
            if (isMeta('sickchill.COMING_EPS_LAYOUT', ['banner', 'poster'])) {
                $(document).ajaxEpSearch();
                $('.ep_summary').hide();
    ...
    addShows: {
        init: function() {
            $('#tabs').tabs({
                collapsible: true,
                selected: (metaToBool('sickchill.SORT_ARTICLE') ? -1 : 0)
            });
        ...
        newShow: function() {
            ...
            const buildTable = function(shows) {
                let table =
                    ...
                    '<table class="sickchillTable new-show-table">' +
                    '<thead>' +
                    '<tr>' +
