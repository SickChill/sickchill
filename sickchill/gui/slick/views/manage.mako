...
<%!
    ...
    import sickchill
    from sickchill.common import statusStrings
%>
            ...
            <div class="col-md-12">
                <div class="horizontal-scroll">
                    <table id="massUpdateTable" class="tablesorter" cellspacing="1" border="0" cellpadding="0">
                        ...
                        <tbody>
                            % for curShow in sorted(sickchill.showList, key=lambda mbr: attrgetter('sort_name')(mbr)):
                            <%
                                if sickchill.showQueueScheduler.action.is_in_remove_queue(curShow) or sickchill.showQueueScheduler.action.is_being_removed(curShow):
                                    continue

                                disabled = sickchill.showQueueScheduler.action.is_being_updated(curShow) or sickchill.showQueueScheduler.action.is_in_update_queue(curShow)
                                curUpdate = "<input type=\"checkbox\" class=\"updateCheck\" id=\"update-" + str(curShow.indexerid) + "\" " + ("", "disabled=\"disabled\" ")[disabled] + "/>"

                                disabled = sickchill.showQueueScheduler.action.is_being_refreshed(curShow) or sickchill.showQueueScheduler.action.is_in_refresh_queue(curShow)
                                curRefresh = "<input type=\"checkbox\" class=\"refreshCheck\" id=\"refresh-" + str(curShow.indexerid) + "\" " + ("", "disabled=\"disabled\" ")[disabled] + "/>"

                                disabled = sickchill.showQueueScheduler.action.is_being_renamed(curShow) or sickchill.showQueueScheduler.action.is_in_rename_queue(curShow)
                                curRename = "<input type=\"checkbox\" class=\"renameCheck\" id=\"rename-" + str(curShow.indexerid) + "\" " + ("", "disabled=\"disabled\" ")[disabled] + "/>"

                                disabled = not curShow.subtitles or sickchill.showQueueScheduler.action.is_being_subtitled(curShow) or sickchill.showQueueScheduler.action.is_in_subtitle_queue(curShow)
                                curSubtitle = "<input type=\"checkbox\" class=\"subtitleCheck\" id=\"subtitle-" + str(curShow.indexerid) + "\" " + ("", "disabled=\"disabled\" ")[disabled] + "/>"

                                disabled = sickchill.showQueueScheduler.action.is_being_renamed(curShow) or sickchill.showQueueScheduler.action.is_in_rename_queue(curShow) or sickchill.showQueueScheduler.action.is_in_refresh_queue(curShow)
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td rowspan="1" colspan="${(15, 16)[bool(sickchill.USE_SUBTITLES)]}" class="align-right alt"><input class="btn pull-right submitMassUpdate" type="button" value="${_('Submit')}" /></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
