    ...
    <tbody>
        % for curDir in dirList:
            <%
                ...
                if curDir['existing_info'][0]:
                    indexer = curDir['existing_info'][2]
                elif sickchill.INDEXER_DEFAULT > 0:
                    indexer = sickchill.INDEXER_DEFAULT
            %>
            <tr>
                ...
                % if curDir['existing_info'][1] and indexer > 0:
                    <td>
                        <a href="${anon_url(sickchill.indexerApi(indexer).config['show_url'], curDir['existing_info'][0])}">${curDir['existing_info'][1]}</a>
                    </td>
                % else:
                    <td>?</td>
                % endif
                <td align="center">
                    <select name="indexer">
                        % for curIndexer in six.iteritems(sickchill.indexerApi().indexers):
