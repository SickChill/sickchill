...
<%!
    ...
    from sickchill import sbdatetime
        ...
        <div class="col-md-12">
            <div class="pull-right key">
                % if 'calendar' != layout:
                    ...
                    % if sickchill.COMING_EPS_DISPLAY_SNATCHED:
            ...
            <div class="pull-right">
                ...
                % else:
                    <label>
                        ...
                        <select name="sort" class="form-control form-control-inline input-sm" onchange="location = this.options[this.selectedIndex].value;" title="Sort">
                            <option value="${srRoot}/setScheduleSort/?sort=date" ${('', 'selected="selected"')[sickchill.COMING_EPS_SORT == 'date']} >${_('Date')}</option>
                            <option value="${srRoot}/setScheduleSort/?sort=network" ${('', 'selected="selected"')[sickchill.COMING_EPS_SORT == 'network']} >${_('Network')}</option>
                            <option value="${srRoot}/setScheduleSort/?sort=show" ${('', 'selected="selected"')[sickchill.COMING_EPS_SORT == 'show']} >${_('Show')}</option>
                        </select>
                        &nbsp;
                    </label>
                % endif
                <label>
                    ...
                    <select name="viewpaused" class="form-control form-control-inline input-sm" onchange="location = this.options[this.selectedIndex].value;" title="View paused">
                        <option value="${srRoot}/toggleScheduleDisplayPaused" ${('', 'selected="selected"')[not bool(sickchill.COMING_EPS_DISPLAY_PAUSED)]}>${_('Hidden')}</option>
                        <option value="${srRoot}/toggleScheduleDisplayPaused" ${('', 'selected="selected"')[bool(sickchill.COMING_EPS_DISPLAY_PAUSED)]}>${_('Shown')}</option>
                    </select>
                    &nbsp;
                </label>
                % if layout != 'calendar':
                <label>
                    ...
                    <select name="viewsnatched" class="form-control form-control-inline input-sm" onchange="location = this.options[this.selectedIndex].value;" title="View snatched">
                        <option value="${srRoot}/toggleScheduleDisplaySnatched" ${('', 'selected="selected"')[not bool(sickchill.COMING_EPS_DISPLAY_SNATCHED)]}>${_('Hidden')}</option>
                        <option value="${srRoot}/toggleScheduleDisplaySnatched" ${('', 'selected="selected"')[bool(sickchill.COMING_EPS_DISPLAY_SNATCHED)]}>${_('Shown')}</option>
                    </select>
                    &nbsp;
                </label>
                % endif
                <label>
                    ...
                    <select name="layout" class="form-control form-control-inline input-sm" onchange="location = this.options[this.selectedIndex].value;" title="Layout">
                        <option value="${srRoot}/setScheduleLayout/?layout=poster" ${('', 'selected="selected"')[sickchill.COMING_EPS_LAYOUT == 'poster']} >${_('Poster')}</option>
                        <option value="${srRoot}/setScheduleLayout/?layout=calendar" ${('', 'selected="selected"')[sickchill.COMING_EPS_LAYOUT == 'calendar']} >${_('Calendar')}</option>
                        <option value="${srRoot}/setScheduleLayout/?layout=banner" ${('', 'selected="selected"')[sickchill.COMING_EPS_LAYOUT == 'banner']} >${_('Banner')}</option>
                        <option value="${srRoot}/setScheduleLayout/?layout=list" ${('', 'selected="selected"')[sickchill.COMING_EPS_LAYOUT == 'list']} >${_('List')}</option>
                    </select>
                </label>
            </div>
        </div>
            ...
            <div class="col-md-12">
                ...
                <div class="horizontal-scroll">
                    <table id="showListTable" class="sickchillTable tablesorter seasonstyle" cellspacing="1" border="0" cellpadding="0">
                        <thead>
                            <tr>
                                <th>${_('Airdate')} (${('local', 'network')[sickchill.TIMEZONE_DISPLAY == 'network']})</th>
                                <th>${_('Ends')}</th>
                                <th>${_('Show')}</th>
                                <th>${_('Next Ep')}</th>
                        ...
                        <tbody style="text-shadow:none;">
                            % for cur_result in results:
                                <%
                                    ...
                                    if int(cur_result[b'paused']) and not sickchill.COMING_EPS_DISPLAY_PAUSED:
                                        continue

                                    if snatched_status and not sickchill.COMING_EPS_DISPLAY_SNATCHED:
                                        continue
                                ...
                                <tr class="${show_div}">
                                    ...
                                    <td align="center" style="vertical-align: middle;">
                                        ...
                                        % endif
                                        <a href="${anon_url(sickchill.indexerApi(cur_indexer).config['show_url'], cur_result[b'showid'])}"
                                           rel="noreferrer" onclick="window.open(this.href, '_blank'); return false"
                                           title="${sickchill.indexerApi(cur_indexer).config['show_url']}${cur_result[b'showid']}">
                                            <img alt="${sickchill.indexerApi(cur_indexer).name}" height="16" width="16"
                                                 src="${static_url('images/indexers/' + sickchill.indexerApi(cur_indexer).config['icon'])}"/>
                                        </a>
                                    </td>
                                    <td align="center">
                    ...
                    <div class="calendarWrapper">
                        ...
                        <% tbl_day += 1 %>
                            <table class="sickchillTable tablesorter calendarTable ${'cal-%s' % (('even', 'odd')[bool(tbl_day % 2)])}"
                                ...
                                <thead>
                                    <tr>
                                        <th>${day.strftime('%A').decode(sickchill.SYS_ENCODING).capitalize()}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <% day_has_show = False %>
                                    % for cur_result in results:
                                        ...
                                        % if airday == day:
                                            % try:
                                                <% day_has_show = True %>
                                                <% airtime = sbdatetime.sbdatetime.fromtimestamp(time.mktime(cur_result[b'localtime'].timetuple())).sbftime().decode(sickchill.SYS_ENCODING) %>
                                                % if sickchill.TRIM_ZERO:
        ...
        % elif layout in ['banner', 'poster']:
            <div class="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-sm-12 col-xs-12">
                ...
                <%
                    ...
                    if snatched_status and (cur_result[b'location'] or not sickchill.COMING_EPS_DISPLAY_SNATCHED):
                        continue
                    ...
                    this_day_name = datetime.date.fromordinal(cur_ep_airdate.toordinal()).strftime('%A').decode(sickchill.SYS_ENCODING).capitalize()
                %>
                % if sickchill.COMING_EPS_SORT == 'network':
                    ...
                    % elif cur_ep_enddate >= today and cur_ep_airdate < next_week.date():
                        ...
                        % endif
                    % endif
                % elif sickchill.COMING_EPS_SORT == 'date':
                    % if snatched_status:
                    ...
                    % elif cur_ep_enddate >= today and cur_ep_airdate < next_week.date():
                        ...
                        % endif
                    % endif
                % elif sickchill.COMING_EPS_SORT == 'show':
                    % if snatched_status:
                                ...
                                <tr>
                                    <td class="next_episode">
                                        ...
                                        <span class="tvshowTitleIcons">
                                            ...
                                            <a href="${anon_url(sickchill.indexerApi(cur_indexer).config['show_url'], cur_result[b'showid'])}"
                                               rel="noreferrer" onclick="window.open(this.href, '_blank'); return false"
                                               title="${sickchill.indexerApi(cur_indexer).config['show_url']}"><img
                                                    alt="${sickchill.indexerApi(cur_indexer).name}" height="16" width="16"
                                                    src="${static_url('images/indexers/' + sickchill.indexerApi(cur_indexer).config['icon'])}"/>
                                            </a>
                                            <span>
                                                <a href="${srRoot}/home/searchEpisode?show=${cur_result[b'showid']}&amp;season=${cur_result[b'season']}&amp;episode=${cur_result[b'episode']}"
