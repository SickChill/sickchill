...
<%!
    ...
    from sickchill import providers, sbdatetime

    from sickchill.common import ARCHIVED, SNATCHED, FAILED, DOWNLOADED, SUBTITLED
            ...
            <div class="pull-right">
                ...
                <label>
                    ...
                    <select name="${_('HistoryLayout')}" class="form-control form-control-inline input-sm" onchange="location = this.options[this.selectedIndex].value;">
                        <option value="${srRoot}/setHistoryLayout/?layout=compact"  ${('', 'selected="selected"')[sickchill.HISTORY_LAYOUT == 'compact']}>${_('Compact')}</option>
                        <option value="${srRoot}/setHistoryLayout/?layout=detailed" ${('', 'selected="selected"')[sickchill.HISTORY_LAYOUT == 'detailed']}>${_('Detailed')}</option>
                    </select>
                </label>
            </div>
        </div>
        ...
        <div class="col-md-12">
            <div class="horizontal-scroll">
                <table id="historyTable" class="sickchillTable tablesorter" cellspacing="1" border="0" cellpadding="0">
                    % if sickchill.HISTORY_LAYOUT == "detailed":
                        <thead>
                            <tr>
                    ...
                    % else:
                        ...
                        <tfoot>
                            <tr>
                                <th class="nowrap" colspan="${("6", "7")[sickchill.USE_SUBTITLES]}">&nbsp;</th>
                            </tr>
                        </tfoot>
                    % endif
                </table>
