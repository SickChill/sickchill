...
<%!
    ...
    from sickchill.show_queue import ShowQueueActions
            ...
            <div class="horizontal-scroll">
                <table id="schedulerStatusTable" class="tablesorter" width="100%">
                    ...
                    <tbody>
                        ...
                        <% service = getattr(sickchill, scheduler) %>
                            <tr>
                                <td>${schedulerName}</td>
                                % if service.isAlive():
                                ...
                                % if scheduler == 'backlogSearchScheduler':
                                    <% searchQueue = getattr(sickchill, 'searchQueueScheduler') %>
        ...
        <div class="col-md-12">
            ...
            <div class="horizontal-scroll">
                <table id="queueStatusTable" class="tablesorter" width="100%">
                    ...
                    <tbody>
                        % if sickchill.showQueueScheduler.action.currentItem is not None:
                            <tr>
                            % try:
                                <% showindexerid = sickchill.showQueueScheduler.action.currentItem.show.indexerid %>
                                <td>${showindexerid}</td>
                            % except Exception:
                                <td></td>
                            % endtry
                            % try:
                                <% showname = sickchill.showQueueScheduler.action.currentItem.show.name %>
                                <td>${showname}</td>
                            % except Exception:
                                % if sickchill.showQueueScheduler.action.currentItem.action_id == ShowQueueActions.ADD:
                                    <td>${sickchill.showQueueScheduler.action.currentItem.showDir}</td>
                                % else:
                                    <td></td>
                                % endif
                            % endtry
                                <td>${sickchill.showQueueScheduler.action.currentItem.inProgress}</td>
                                % if sickchill.showQueueScheduler.action.currentItem.priority == 10:
                                    <td>${_('LOW')}</td>
                                % elif sickchill.showQueueScheduler.action.currentItem.priority == 20:
                                    <td>${_('NORMAL')}</td>
                                % elif sickchill.showQueueScheduler.action.currentItem.priority == 30:
                                    <td>${_('HIGH')}</td>
                                % else:
                                    <td>sickchill.showQueueScheduler.action.currentItem.priority</td>
                                % endif
                                <td>${sickchill.showQueueScheduler.action.currentItem.added.strftime(dateTimeFormat)}</td>
                                <td>${ShowQueueActions.names[sickchill.showQueueScheduler.action.currentItem.action_id]}</td>
                            </tr>
                        % endif
                        % for item in sickchill.showQueueScheduler.action.queue:
                            <tr>
                            % try:
        ...
        <div class="col-md-12">
            ...
            <div class="horizontal-scroll">
                <table id="DFStatusTable" class="tablesorter" width="50%">
                    ...
                    <tbody>
                        % if sickchill.TV_DOWNLOAD_DIR:
                            <tr>
                                <td>${_('TV Download Directory')}</td>
                                <td>${sickchill.TV_DOWNLOAD_DIR}</td>
