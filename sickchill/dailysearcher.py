...
import sickchill
from sickchill import common, db, logger, network_timezones
...
class DailySearcher(object):  # pylint:disable=too-few-public-methods
    ...
    def run(self, force=False):  # pylint:disable=too-many-branches
        ...
        for sqlEp in sql_results:
            try:
                if not show or int(sqlEp[b"showid"]) != show.indexerid:
                    show = Show.find(sickchill.showList, int(sqlEp[b"showid"]))
        ...
        dailysearch_queue_item = sickchill.search_queue.DailySearchQueueItem()
        sickchill.searchQueueScheduler.action.add_item(dailysearch_queue_item)
