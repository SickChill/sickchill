...
import sickchill
from sickchill import db, helpers, image_cache, logger, network_timezones, notifiers, postProcessor, subtitles
...
class TVShow(object):  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    def __init__(self, indexer, indexerid, lang=""):
        ...
        self._runtime = 0
        self._imdb_info = {}
        self._quality = int(sickchill.QUALITY_DEFAULT)
        self._season_folders = int(sickchill.SEASON_FOLDERS_DEFAULT)
        self._status = "Unknown"
        self._airs = ""
        self._startyear = 0
        self._paused = 0
        self._air_by_date = 0
        self._subtitles = int(sickchill.SUBTITLES_DEFAULT)
        ...
        otherShow = Show.find(sickchill.showList, self.indexerid)
    ...
    def _getLocation(self):
        ...
        if sickchill.CREATE_MISSING_SHOW_DIRS or ek(os.path.isdir, self._location):
    ...
    def _setLocation(self, newLocation):
        ...
        if sickchill.ADD_SHOWS_WO_DIR or ek(os.path.isdir, newLocation):
    ...
    def loadEpisodesFromDB(self):  # pylint: disable=too-many-locals
        ...
        lINDEXER_API_PARMS = sickchill.indexerApi(self.indexer).api_params.copy()

        lINDEXER_API_PARMS['language'] = self.lang or sickchill.INDEXER_DEFAULT_LANGUAGE
        ...
        # Spamming log
        t = sickchill.indexerApi(self.indexer).indexer(**lINDEXER_API_PARMS)
        ...
        for curResult in sql_results:
            ...
            if curSeason not in cachedSeasons:
                ...
                except sickchill.indexer_seasonnotfound as error:
                    logger.log("{0}: {1} (unaired/deleted) in the indexer {2} for {3}. Removing existing records from database".format
                               (curShowid, error.message, sickchill.indexerApi(self.indexer).name, curShowName), logger.DEBUG)
                    deleteEp = True
    ...
    def loadEpisodesFromIndexer(self, cache=True):
        ...
        try:
            ...
            logger.log("" + sickchill.indexerApi(self.indexer).name +
                       " timed out, unable to update episodes from " +
                       sickchill.indexerApi(self.indexer).name, logger.WARNING)
            return None

        logger.log(
            str(self.indexerid) + ": Loading all episodes from " + sickchill.indexerApi(self.indexer).name + "..", logger.DEBUG)
            ...
            for episode in showObj[season]:
                ...
                try:
                    ...
                    logger.log("{id}: {indexer} object for {ep} is incomplete, skipping this episode".format
                               (id=self.indexerid, indexer=sickchill.indexerApi(self.indexer).name, ep=episode_num(season, episode)))
                    continue
                else:
                    try:
                ...
                with ep.lock:
                    ...
                    #            (id=self.indexerid, indexer=sickchill.indexerApi(self.indexer).name,
                    #             ep=episode_num(season, episode)), logger.DEBUG)
    ...
    def makeEpFromFile(self, filepath):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        for current_ep in episodes:
            ...
            elif not same_file and sickchill.helpers.is_media_file(filepath) and curEp.status not in Quality.DOWNLOADED + Quality.ARCHIVED + [IGNORED]:
    ...
    def loadFromIndexer(self, cache=True, tvapi=None):
        ...
        logger.log(str(self.indexerid) + ": Loading show info from " + sickchill.indexerApi(self.indexer).name, logger.DEBUG)
        ...
        try:
            ...
            raise sickchill.indexer_attributenotfound(
    ...
    def loadIMDbInfo(self):  # pylint: disable=too-many-branches
        ...
        if sickchill.PROXY_SETTING and sickchill.PROXY_INDEXERS:
            i = imdb.IMDb(proxy=sickchill.PROXY_SETTING)
        else:
            i = imdb.IMDb()
    ...
    def deleteShow(self, full=False):
        ...
        action = ('delete', 'trash')[sickchill.TRASH_REMOVE_SHOW]

        # remove self from show list
        sickchill.showList = [x for x in sickchill.showList if int(x.indexerid) != self.indexerid]

        # clear the cache
        image_cache_dir = ek(os.path.join, sickchill.CACHE_DIR, 'images')
        for cache_file in ek(glob.glob, ek(os.path.join, glob.escape(image_cache_dir), str(self.indexerid) + '.*')):
            ...
            try:
                if sickchill.TRASH_REMOVE_SHOW:
            ...
            try:
                ...
                if num_shows_in_folder:
                    ...
                    logger.log('{0} show folder {1}'.format
                               (('Deleted', 'Trashed')[sickchill.TRASH_REMOVE_SHOW], self._location))
    ...
    def refreshDir(self):
        ...
        if not ek(os.path.isdir, self._location) and not sickchill.CREATE_MISSING_SHOW_DIRS:
            return False
        ...
        for ep in sql_results:
            ...
            if not ek(os.path.isfile, curLoc) or not ek(os.path.normpath, curLoc).startswith(
                ...
                if not sickchill.SKIP_REMOVED_FILES:
                    with curEp.lock:
                        # if it used to have a file associated with it and it doesn't anymore then set it to sickchill.EP_DEFAULT_DELETED_STATUS
                        if curEp.location and curEp.status in Quality.DOWNLOADED:

                            if sickchill.EP_DEFAULT_DELETED_STATUS == ARCHIVED:
                                oldStatus_, oldQuality = Quality.splitCompositeStatus(curEp.status)
                                new_status = Quality.compositeStatus(ARCHIVED, oldQuality)
                            else:
                                new_status = sickchill.EP_DEFAULT_DELETED_STATUS
...
class TVEpisode(object):  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    def loadFromDB(self, season, episode):  # pylint: disable=too-many-branches
        ...
        if len(sql_results) > 1:
            ...
            sickchill.scene_numbering.xem_refresh(self.show.indexerid, self.show.indexer)
            ...
            if self.scene_absolute_number == 0:
                self.scene_absolute_number = sickchill.scene_numbering.get_scene_absolute_numbering(
                    self.show.indexerid,
                    self.show.indexer,
            ...
            if self.scene_season == 0 or self.scene_episode == 0:
                self.scene_season, self.scene_episode = sickchill.scene_numbering.get_scene_numbering(
                    self.show.indexerid,
                    self.show.indexer,
    ...
    def loadFromIndexer(self, season=None, episode=None, cache=True, tvapi=None, cachedSeason=None):  # pylint: disable=too-many-arguments, too-many-branches, too-many-statements
        ...
        #             indexer=sickchill.indexerApi(self.show.indexer).name), logger.DEBUG)
        ...
        try:
        ...
        except (sickchill.indexer_error, IOError) as error:
            logger.log("" + sickchill.indexerApi(self.indexer).name + " threw up an error: " + ex(error), logger.DEBUG)
            ...
            if self.name:
                ...
                logger.log("" + sickchill.indexerApi(self.indexer).name + " timed out, unable to create the episode",
                           logger.ERROR)
                return False
        except (sickchill.indexer_episodenotfound, sickchill.indexer_seasonnotfound):
            logger.log("Unable to find the episode on " + sickchill.indexerApi(
        ...
        if getattr(myEp, 'episodename', None) is None:
            logger.log("This episode {show} - {ep} has no name on {indexer}. Setting to an empty string".format
                       (show=self.show.name, ep=episode_num(season, episode), indexer=sickchill.indexerApi(self.indexer).name))
        ...
        if getattr(myEp, 'absolute_number', None) is None:
            logger.log("{id}: This episode {show} - {ep} has no absolute number on {indexer}".format
                       (id=self.show.indexerid, show=self.show.name, ep=episode_num(season, episode),
                        indexer=sickchill.indexerApi(self.indexer).name), logger.DEBUG)
        else:
        ...
        try:
            ...
            logger.log("Malformed air date of {aired} retrieved from {indexer} for ({show} - {ep})".format
                       (aired=firstaired, indexer=sickchill.indexerApi(self.indexer).name, show=self.show.name,
        ...
        if self.indexerid is None:
            logger.log("Failed to retrieve ID from {indexer}".format
                       (indexer=sickchill.indexerApi(self.indexer).name), logger.ERROR)
            if self.indexerid != -1:
                self.deleteEpisode()
            return False

        # don't update show status if show dir is missing, unless it's missing on purpose
        if not ek(os.path.isdir, self.show._location) and not sickchill.CREATE_MISSING_SHOW_DIRS and not sickchill.ADD_SHOWS_WO_DIR:  # pylint: disable=protected-access
        ...
        elif sickchill.helpers.is_media_file(self.location):
    ...
    def loadFromNFO(self, location):  # pylint: disable=too-many-branches
        ...
        if self.location != "":
            ...
            if self.status == UNKNOWN and sickchill.helpers.is_media_file(self.location):
    ...
    def get_sql(self, forceSave=False):
        ...
        try:
            ...
            if epID:
                ...
                if sickchill.SUBTITLES_MULTI or not rows[0][b'subtitles'] or not self.subtitles:
                    return [
    ...
    def _replace_map(self):  # pylint: disable=too-many-branches
        ...
        if sickchill.NAMING_STRIP_YEAR:
    ...
    def _format_pattern(self, pattern=None, multi=None, anime_type=None):  # pylint: disable=too-many-branches, too-many-statements, too-many-locals
        ...
        if pattern is None:
            pattern = sickchill.NAMING_PATTERN

        if multi is None:
            multi = sickchill.NAMING_MULTI_EP

        if sickchill.NAMING_CUSTOM_ANIME:
            if anime_type is None:
                anime_type = sickchill.NAMING_ANIME
        else:
            anime_type = 3
    ...
    def proper_path(self):
        ...
        if not (self.show.season_folders or sickchill.NAMING_FORCE_FOLDERS):
            return result
    ...
    def formatted_dir(self, pattern=None, multi=None, anime_type=None):
        ...
        if pattern is None:
            ...
            if self.show.air_by_date and sickchill.NAMING_CUSTOM_ABD and not self.relatedEps:
                pattern = sickchill.NAMING_ABD_PATTERN
            elif self.show.sports and sickchill.NAMING_CUSTOM_SPORTS and not self.relatedEps:
                pattern = sickchill.NAMING_SPORTS_PATTERN
            elif self.show.anime and sickchill.NAMING_CUSTOM_ANIME:
                pattern = sickchill.NAMING_ANIME_PATTERN
            else:
                pattern = sickchill.NAMING_PATTERN
    ...
    def rename(self):  # pylint: disable=too-many-locals, too-many-branches
        ...
        # get related subs
        if self.show.subtitles and sickchill.SUBTITLES_DIR:
            # assume that the video file is in the subtitles dir to find associated subs
            subs_path = os.path.join(sickchill.SUBTITLES_DIR, ek(os.path.basename, self.location))
        ...
        if sickchill.MOVE_ASSOCIATED_FILES:
            # move related files
            ...
            for cur_related_sub in related_subs:
                absolute_proper_subs_path = ek(os.path.join, sickchill.SUBTITLES_DIR, self.formatted_filename())
    ...
    def airdateModifyStamp(self):
        ...
        """
        if not all([sickchill.AIRDATE_EPISODES, self.airdate, self.location,
        ...
        try:
            ...
            if sickchill.FILE_TIMESTAMP_TIMEZONE == 'local':
