...
def sendNZB(nzb, proper=False):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches, too-many-return-statements
    ...
    '''
    if sickchill.NZBGET_HOST is None:
        logger.log('No NZBget host found in configuration. Please configure it.', logger.WARNING)
        return False

    addToTop = False
    nzbgetprio = 0
    category = sickchill.NZBGET_CATEGORY
    if nzb.show.is_anime:
        category = sickchill.NZBGET_CATEGORY_ANIME

    url = 'http{0}://{1}:{2}@{3}/xmlrpc'.format(
        's' if sickchill.NZBGET_USE_HTTPS else '',
        sickchill.NZBGET_USERNAME,
        sickchill.NZBGET_PASSWORD,
        sickchill.NZBGET_HOST)
    ...
    for curEp in nzb.episodes:
        ...
        if datetime.date.today() - curEp.airdate <= datetime.timedelta(days=7):
            addToTop = True
            nzbgetprio = sickchill.NZBGET_PRIORITY
        else:
            category = sickchill.NZBGET_CATEGORY_BACKLOG
            if nzb.show.is_anime:
                category = sickchill.NZBGET_CATEGORY_ANIME_BACKLOG
