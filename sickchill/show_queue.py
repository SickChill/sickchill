...
class ShowQueueItem(generic_queue.QueueItem):
    ...
    def is_in_queue(self):
        return self in sickchill.showQueueScheduler.action.queue + [
            sickchill.showQueueScheduler.action.currentItem]
...
class QueueItemAdd(ShowQueueItem):  # pylint: disable=too-many-instance-attributes
    ...
    def is_loading(self):
        ...
        """
        return self.show not in sickchill.showList or not self.show
    ...
    def run(self):  # pylint: disable=too-many-branches, too-many-statements, too-many-return-statements
        ...
        try:
            ...
            logger.log('{0}: {1!r}'.format(sickchill.indexerApi(self.indexer).name, lINDEXER_API_PARMS))
            ...
            if getattr(s, 'seriesname', None) is None:
                ...
                error_string = 'Show in {0} has no name on {1}, probably searched with the wrong language. Delete .nfo and add manually in the correct language.'.format(
                    self.showDir, sickchill.indexerApi(self.indexer).name)
            ...
            if not s:
                error_string = 'Show {0} is on {1} but contains no season/episode data.'.format(
                    s[b'seriesname'], sickchill.indexerApi(self.indexer).name)
            ...
            error_string = 'Unable to look up the show in {0} on {1} using ID {2}, not using the NFO. Delete .nfo and try adding manually again.'.format(
                self.showDir, sickchill.indexerApi(self.indexer).name, self.indexer_id)
            ...
            if sickchill.USE_TRAKT:
                trakt_id = sickchill.indexerApi(self.indexer).config[b'trakt_id']
        ...
        try:
            try:
                ...
                existing_show = Show.find(sickchill.showList, self.indexer_id)
                # noinspection PyProtectedMember
            ...
            self.show.subtitles = self.subtitles if self.subtitles is not None else sickchill.SUBTITLES_DEFAULT
            self.show.subtitles_sr_metadata = self.subtitles_sr_metadata
            self.show.quality = self.quality if self.quality else sickchill.QUALITY_DEFAULT
            self.show.season_folders = self.season_folders if self.season_folders is not None else sickchill.SEASON_FOLDERS_DEFAULT
            self.show.anime = self.anime if self.anime is not None else sickchill.ANIME_DEFAULT
            self.show.scene = self.scene if self.scene is not None else sickchill.SCENE_DEFAULT
        ...
        except sickchill.indexer_exception as error:
            error_string = 'Unable to add {0} due to an error with {1}'.format(
                self.show.name if self.show else 'show', sickchill.indexerApi(self.indexer).name)
        ...
        if not Show.find(sickchill.showList, self.indexer_id):
            sickchill.showList.append(self.show)
        ...
        try:
            ...
            logger.log(
                ...
                (sickchill.indexerApi(self.show.indexer).name, error), logger.ERROR)
            logger.log(traceback.format_exc(), logger.DEBUG)
        ...
        if self.show.default_ep_status == WANTED:
            ...
            sickchill.backlogSearchScheduler.action.searchBacklog([self.show])
        ...
        if sickchill.USE_TRAKT:
            ...
            sickchill.traktCheckerScheduler.action.manageNewShow(self.show)
            # add show to trakt.tv library
            if sickchill.TRAKT_SYNC:
                sickchill.traktCheckerScheduler.action.addShowToTraktLibrary(self.show)
    ...
    def _finish_early(self):
        if self.show is not None:
            sickchill.showQueueScheduler.action.remove_show(self.show)
...
class QueueItemUpdate(ShowQueueItem):
    ...
    def run(self):  # pylint: disable=too-many-branches, too-many-statements
        ...
        logger.log('Retrieving show info from {0}'.format(sickchill.indexerApi(self.show.indexer).name), logger.DEBUG)
        try:
            self.show.loadFromIndexer(cache=not self.force)
        except sickchill.indexer_error as error:
            logger.log('Unable to contact {0}, aborting: {1}'.format
                       (sickchill.indexerApi(self.show.indexer).name, error), logger.WARNING)
            super(QueueItemUpdate, self).finish()
            self.finish()
            return
        except sickchill.indexer_attributenotfound as error:
        ...
        logger.log('Loading all episodes from {0}'.format(sickchill.indexerApi(self.show.indexer).name), logger.DEBUG)
        try:
        ...
        if not IndexerEpList:
            logger.log('No data returned from {0}, unable to update this show.'.format
                       (sickchill.indexerApi(self.show.indexer).name), logger.ERROR)
        else:
        ...
        sickchill.showQueueScheduler.action.refresh_show(self.show, self.force)
        super(QueueItemUpdate, self).finish()
        self.finish()
...
class QueueItemRemove(ShowQueueItem):
    ...
    def run(self):
        ...
        if sickchill.USE_TRAKT:
            try:
                sickchill.traktCheckerScheduler.action.removeShowFromTraktLibrary(self.show)
            except Exception as error:
