...
class PiSexyProvider(TorrentProvider):

    def __init__(self):

        TorrentProvider.__init__(self, "PiSexy")

        self.username = None
        self.passkey = None

        self.cache = PiSexyCache(self, min_time=15)  # only poll PiSexy every 15 minutes max

        self.url = 'https://pisexy.me'
        self.urls = {
            'search': urljoin(self.url, '/sickbeard'),
            'rss': urljoin(self.url, '/sickbeard'),
...
class PiSexyCache(tvcache.TVCache):
    def _getRSSData(self):
        self.search_params = None  # PiSexy cache does not use search_params so set it to None
        results = []
