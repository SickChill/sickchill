...
API_CLIENT_REGISTERED_URL='http://devclinton.com'

class PutioAPI(GenericClient):
    def __init__(self, host=None, username=None, password=None):
        super(PutioAPI, self).__init__('put_io', host, username, password)
        self.url = '{}/oauth2/authenticate?client_id={}&response_type=token&redirect_uri={}'.format(PUTIO_BASE_URL,API_CLIENT_ID, urllib.urlencode(API_CLIENT_REGISTERED_URL))
    ...
    def _get_auth(self):
        ...
        post_data = {
            ...
            'next': '/v2/oauth2/authenticate?client_id={}&response_type=token&redirect_uri=http://devclinton.com'.format(API_CLIENT_ID)
        }
        try:
            ...
            self.auth = re.search('{}#access_token=(.*)'.format(API_CLIENT_REGISTERED_URL), self.tok_response.headers['location']).group(1)
        except Exception:
    ...
    def _add_torrent_file(self, result):
        ...
        self.response = self.session.post('{}/files/upload?oauth_token={}'.format(PUTIO_BASE_URL,self.access_token),  data=post_data, files=('putio_torrent',result.name), params=params)
